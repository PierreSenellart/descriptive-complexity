...
`DescriptiveComplexity.Pfp.gateMat` is the gated matrix and
`DescriptiveComplexity.Pfp.altQuantFrom_gateMat` the equivalence. The two
...
(`DescriptiveComplexity.Pfp.not_altQuantFrom_gateMat_of_bad_ex`), one under a
universal polarity satisfies it outright
(`DescriptiveComplexity.Pfp.altQuantFrom_gateMat_of_bad_all`) – which is
