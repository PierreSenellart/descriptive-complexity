...
`DescriptiveComplexity.Pfp.PfpData.nexTotal_lt_two_pow'` asks for each factor
below `2 ^ (k * m)`. The rounds are bounded elsewhere
(`DescriptiveComplexity.Pfp.PfpData.chain_length_le_two_pow`: the VAL loop is an
...
`DescriptiveComplexity.Pfp.PfpData.ixLegWidth` times the number of spine
positions.
...
it too. -/
structure IxWidthBd (dt : PfpData L) (A : Type) (w wP wR wK q : ℕ) : Prop where
...
(`DescriptiveComplexity.Pfp.PfpData.ixSpineCost_le_mul`); the second is the
number of VAL rounds, bounded by
`DescriptiveComplexity.Pfp.PfpData.chain_length_le_two_pow`. -/
