...
`DescriptiveComplexity.Pfp.PfpData.ixBack` reads a background at an arbitrary
...
cell*, so `DescriptiveComplexity.Pfp.TapeSt` takes the index and
`DescriptiveComplexity.Pfp.TapeStD` is the elementwise case.
`DescriptiveComplexity.Pfp.PfpData.back` is that at the
...
(`DescriptiveComplexity.Pfp.PfpData.ixBack_regFirst`,
`DescriptiveComplexity.Pfp.PfpData.ixBack_regLast`): which cell carries them is
...
structure TapeSt (dt : PfpData L) (A R' P' I : Type) : Type 1 where
  /-- The MIRROR register. -/
  mir : I → Prop
  /-- The TARGET register. -/
  tgt : I → Prop
...
abbrev TapeStD (dt : PfpData L) (A R' P' : Type) : Type 1 :=
...
stops at. -/
def padTup {L : Language.{0, 0}} {dt : PfpData L} {A : Type}
...
structure Layout (dt : PfpData L) (A R' P' I : Type) : Type where
...
def NameSep {L : Language.{0, 0}} {dt : PfpData L} {A R' P' I : Type}
...
The tuples asked for are the **named** ones, `DescriptiveComplexity.Pfp.padTup`
...
def HasName {L : Language.{0, 0}} {dt : PfpData L} {A R' P' I : Type}
...
noncomputable def reg {L : Language.{0, 0}} {dt : PfpData L} {A R' P' I : Type}
...
theorem blk_reg {L : Language.{0, 0}} {dt : PfpData L} {A R' P' I : Type}
    {lay : Layout dt A R' P' I} {zero : A} (hhas : lay.HasName zero)
    (b : Fin dt.ko ⊕ Fin dt.ki) (c : Fin dt.dd0 → A) :
...
theorem arg_reg {L : Language.{0, 0}} {dt : PfpData L} {A R' P' I : Type}
    {lay : Layout dt A R' P' I} {zero : A} (hhas : lay.HasName zero)
    (b : Fin dt.ko ⊕ Fin dt.ki) (c : Fin dt.dd0 → A) :
...
(`DescriptiveComplexity.Pfp.LaidFile.toIxFile`) and the background for the
layout alone (`DescriptiveComplexity.Pfp.LaidFile.toLayout`); carrying the two
together is what lets one parameter stand where
`DescriptiveComplexity.RegFile` stood. -/
structure LaidFile (dt : PfpData L) (A R' P' I : Type)
...
its own tuple. -/
@[reducible] noncomputable def laidFile {L : Language.{0, 0}} {dt : PfpData L} {A R' P' : Type}
    [LinearOrder A] [LinearOrder R'] [LinearOrder P']
...
theorem isLinOrd_laidFile_le {L : Language.{0, 0}} {dt : PfpData L} {A R' P' : Type}
    [LinearOrder A] [LinearOrder R'] [LinearOrder P']
    [Language.wide.Structure (Univ A R' P' dt.KIx dt.dd)]
    {F : RegFile (Univ A R' P' dt.KIx dt.dd)}
    {hord : ∀ x y : Univ A R' P' dt.KIx dt.dd, WMLe x y ↔ tagTupleLe x y}
  ...
  show IsLinOrd (tagTupleLe (A := A) (Tag := PfpTag R' P' dt.KIx)) from
...
carved out of this by `DescriptiveComplexity.Pfp.Prog.trackTapeAt`; everything
else rides along.
...
else about the index. `DescriptiveComplexity.PfpData.back` is this at the
...
`DescriptiveComplexity.Pfp.LaidFile`, so that the runs stated at an arbitrary
file apply to it – its layout is `DescriptiveComplexity.Pfp.PfpData.diagLayout`
on the nose, so its background is `DescriptiveComplexity.Pfp.PfpData.back`. -/
...
/-- **The state, presented as a background**: `DescriptiveComplexity.PfpData.ixBack`
...
below are their instances at `DescriptiveComplexity.Pfp.PfpData.diagLayout`,
...
`DescriptiveComplexity.Pfp.Layout.NameSep`, that the marks tell the registers
...
theorem ixNameGF_cell (hzo : zero ≠ one) (hinj : Function.Injective lay.cell)
  ...
  rw [PfpData.nameGF, dt.ixBack_blk_cell hinj, dt.ixBack_pdd_cell hinj]
...
(`DescriptiveComplexity.Pfp.Layout.NameSep`) and which has a register for every
name (`DescriptiveComplexity.Pfp.Layout.HasName`), the guard of a scan by name
holds at **one** register of the file and nowhere else on it. That is exactly
the pair of hypotheses `DescriptiveComplexity.Pfp.Prog.reaches_toCell` asks
...
file: `DescriptiveComplexity.Pfp.SeekKit` steps a marker until the walked
...
read straight off the control: `DescriptiveComplexity.Pfp.PfpData.ixNameGF_iff`
...
omit [LinearOrder A] [LinearOrder R'] [LinearOrder P']
  ...
  [Language.wide.Structure (Univ A R' P' dt.KIx dt.dd)] in
/-- **A named tuple is the padded one**: `DescriptiveComplexity.Pfp.padTup` and
`DescriptiveComplexity.Pfp.pad` are the same function, written once for the name
...
theorem hasName_diagLayout : (dt.diagLayout cell).HasName zero := by
  intro b c
  refine ⟨(PfpTag.arg (toLex b), padTup zero c), ?_, rfl⟩
  exact (tagBlk_eq_some_iff (R := R') (P := P') (PfpTag.arg (toLex b)) b).mpr rfl
