...
* `DescriptiveComplexity.Pfp.PreRule` – a rule without its source phase,
  what a dispatch descriptor is;
* `DescriptiveComplexity.Pfp.ChainPh`/`ChainSite`/`ChainSh` – `n`
  checkpoints (each carrying the three-rule gadget of
  `DescriptiveComplexity.Pfp.EvalChkRule`) plus an abstract stage family;
* `DescriptiveComplexity.Pfp.chainRule` – the rules, the dispatches given
  as **descriptors** per checkpoint (`Fin n → Bool → PreRule`): a
  descriptor's target may be any phase, so loops – a tuple loop's back edge
  – cost nothing;
* `DescriptiveComplexity.Pfp.chainSep`/`chainHosrc` – separation and
