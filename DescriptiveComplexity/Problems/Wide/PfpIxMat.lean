...
`DescriptiveComplexity.Problems.Wide.PfpInstMat` read at a coarse file: the
...
`DescriptiveComplexity.Pfp.PfpData.ixStage_hStage` with *no* boundary
...
`DescriptiveComplexity.Pfp.PfpData.ixStageEndSt`. -/
...
theorem ixGateBlock_hStage_pos (t : dt.X.Tag)
    (htag : dt.dspTagOf PR.zero PR.one
      (wmBlk (ixAddr elt st.mir) (PfpTag.arg (toLex b) : PfpTag R P dt.KIx)) = t)
