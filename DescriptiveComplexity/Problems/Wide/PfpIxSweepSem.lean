...
The half of `DescriptiveComplexity.Problems.Wide.PfpIxSpineSem` that only a
...
of `DescriptiveComplexity.Pfp.PfpData.ixGatedSem`, quantified over the address
as well: `DescriptiveComplexity.Pfp.PfpData.ixStEndB` runs the spine at
...
`DescriptiveComplexity.Pfp.PfpData.ixEvalSpine_run` asks, at every address, for
`hsavOf`/`htgtOf` — the SAV and TARGET registers holding *that address* —
because `DescriptiveComplexity.Pfp.PfpData.ixMatrix_run` reads them there. But
