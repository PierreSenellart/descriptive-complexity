...
`DescriptiveComplexity.Pfp.PfpData.gateArgs`. The gate asks of one block of
...
at the **decoded** assignment `DescriptiveComplexity.Pfp.decRho` – the raw
block value, no encoding assumed, which is what lets the gate be the check
rather than presuppose it.

The layer mirrors `DescriptiveComplexity.Problems.Wide.PfpInstExp` with two
