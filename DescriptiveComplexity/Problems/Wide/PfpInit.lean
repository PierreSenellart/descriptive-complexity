...
`DescriptiveComplexity.Pfp.Prog.trackTapeAt` presentation. This file joins them.
...
file and `DescriptiveComplexity.Pfp.Prog.isInit_prog` at the channel's, and this
...
`DescriptiveComplexity.Pfp.Prog.isInit_prog` says, run for fewer steps than there
...
`DescriptiveComplexity.Pfp.Prog.dwideAcceptSpace_prog`, which asks for
determinism. -/
