...
The mirror of `DescriptiveComplexity.Problems.Wide.PfpIxGate` at the **VAL
...
(`DescriptiveComplexity.Pfp.PfpData.ixGateTagCell`/`ixGateECell`), so only the
...
address holds. -/
noncomputable def ixIGateLeafP (dt : PfpData L) {I : Type}
...
variable (htag : dt.dspTagOf PR.zero PR.one
  (wmBlk (ixAddr eltR st.val) (PfpTag.arg (toLex b) : PfpTag R P dt.KIx)) = t)
      ...
      (dt.ctlBit one f₀ flag ∧
        ...
        ExpExpansion.DomHolds (X := dt.X)
          (t, decRho dt.ly zero one
            (wmBlk (ixAddr elt st.val) (PfpTag.arg (toLex b) : PfpTag R P dt.KIx)))) := by
