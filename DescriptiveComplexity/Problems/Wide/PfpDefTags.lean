...
(`DescriptiveComplexity.Pfp.PfpData.GateTagsAre`) is one-hotness of finitely
...
`DescriptiveComplexity.Pfp.PfpData.defTag` is chosen from a *nonemptiness* of
...
into the value – and `DescriptiveComplexity.Pfp.PfpData.uConst_defTag` is that
