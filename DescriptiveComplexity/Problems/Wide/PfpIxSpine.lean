...
`DescriptiveComplexity.Problems.Wide.PfpIxEval`'s legs folded by the
...
plugs in `DescriptiveComplexity.Pfp.PfpData.ixVarLeg_run` at a gated address
and `DescriptiveComplexity.Pfp.PfpData.ixVarLegFail_run` at a junk one. -/
...
`DescriptiveComplexity.Pfp.PfpData.ixVarLeg_run`, the tape and control
...
`DescriptiveComplexity.Pfp.PfpData.ixEvalSpine_run` with `hsavOf` and
...
`DescriptiveComplexity.Pfp.PfpData.ixEvalSpine_run_thread` with the gates no
