...
`DescriptiveComplexity.Pfp.PfpData.ixBack` of some tape state only because no
...
`DescriptiveComplexity.Pfp.PfpData.ixBack`, whose tracks are `bitVal`s. -/
...
variable {L : Language.{0, 0}} {dt : PfpData L} {A Q B G : Type}
...
variable {L : Language.{0, 0}} {dt : PfpData L} {A R P K : Type} {c dd : ℕ}
...
theorem step_tape_of_shape_tr (hR : PR.table.Reads)
    ...
    (hread : (wideData (Univ A R P K dd)).Read
      (Sum.inr ((PfpTag.ctrl r : PfpTag R P K), w)) (x.tape x.head))
    (hwrite : (wideData (Univ A R P K dd)).Write
      (Sum.inr ((PfpTag.ctrl r : PfpTag R P K), w)) (y.tape x.head))
  ...
  have hxv : x.tape x.head = Sum.inr (PR.syElt (rest v)) := by
    ...
    rfl
  rw [hxv] at hread
  have hread' : PR.table.Read (PfpTag.ctrl r, w) (PR.syElt (rest v)) :=
  ...
  · rw [hy] at hwrite
    exact hwrite.elim
  · rw [hy] at hwrite
    have hwrite' : PR.table.Write (PfpTag.ctrl r, w) a' := (hR.write _ _).mp hwrite
