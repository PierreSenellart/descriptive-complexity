...
`DescriptiveComplexity.Problems.Wide.PfpSpineSem` read at a coarse file: what a
...
-- The marks-to-shapes bridge at a coarse file (see `PfpIxRoundSem`).
...
-- (`DescriptiveComplexity.Pfp.PfpData.isEnc_of_gatedAt` and its converse).
...
is not literally a
`DescriptiveComplexity.Pfp.PfpData.ixPostVarSt` of the position's entry
state (its VAL loop may normalize the two scratch registers first), while
this projection of it is
(`DescriptiveComplexity.Pfp.PfpData.ixLegStB_new`). -/
abbrev IxWritesNew
...
(`DescriptiveComplexity.Pfp.PfpData.ixVarLegFail_run`,
`DescriptiveComplexity.Pfp.PfpData.ixVarLegUngated_run`) store `False`, which
...
`DescriptiveComplexity.Pfp.PfpData.ixAccVerdict_next` reads as
...
`DescriptiveComplexity.Pfp.PfpData.ixSpineNode`, with the leg's own exit
...
(`DescriptiveComplexity.Pfp.PfpData.ixLegStB`), and is otherwise the same
...
the *conditioned* family `DescriptiveComplexity.Pfp.PfpData.ixGatedSem`
...
`DescriptiveComplexity.Pfp.PfpData.ixStEndB` runs this spine at `v := w` for
...
lemmas ask of a leg (`DescriptiveComplexity.Pfp.PfpData.ixLegStB_new`). -/
...
twin of `DescriptiveComplexity.Pfp.PfpData.ixSpineRide` at the legs the
...
(`DescriptiveComplexity.Pfp.PfpData.nexIxEvalOutB_reachesIn`) asks for, and the
...
`DescriptiveComplexity.Pfp.PfpData.ixSweepStEG_mir` for an evaluation that
