...
**payload** laid out at fixed coordinates (`DescriptiveComplexity.Pfp.EncLayout`
fixes the coordinates, `DescriptiveComplexity.Pfp.encTup` writes one tuple):
...
`DescriptiveComplexity.Pfp.encTup_code_inj` reads the code back and
`DescriptiveComplexity.Pfp.encPt_injective` makes the whole encoding injective;
`DescriptiveComplexity.Pfp.mem_encPt_asg` reads one bit of the assignment off
one membership question, which is the form the machine's register lookups take.
`DescriptiveComplexity.Pfp.IsEnc` – *being* the encoding of a point of the
expanded universe – is the gate the relativization lemma of
`DescriptiveComplexity.Problems.Wide.PfpRel` asks for
(`DescriptiveComplexity.Pfp.isEnc_iff`).
-/
