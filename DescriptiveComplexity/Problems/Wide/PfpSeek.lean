...
`DescriptiveComplexity.Pfp.Prog.reaches_fileSeekTo` is one theorem: from the
...
(`DescriptiveComplexity.Pfp.Prog.reachesIn_ixSeekTo`), because a clocked program
...
`DescriptiveComplexity.Pfp.Prog.reaches_fileRoundTrip` around the file test
(`DescriptiveComplexity.Pfp.Prog.reaches_fileTestG` at *the walked mirror digit
agrees with the target digit*), and – the test having failed below the
target – an `DescriptiveComplexity.Pfp.Prog.reaches_fileAdvance`; the loop is
...
theorem reachesIn_fileSeekTo (F : RegFile (Univ A R P K dd)) (hR : PR.table.Reads)
    ...
    (hstayA : ∀ g : W → A, g rg ≠ PR.one → g wk ≠ PR.one →
      PR.HasLeft pA3 fc g pA3 fc g)
    (hbackC : ∀ g : W → A, g wk ≠ PR.one → PR.HasLeft pChk fc g pChk fc g)
    {w : ℕ} (hgap : ∀ u u' : Univ A R P K dd, WMSucc (Univ A R P K dd) u u' →
      wideRank (F.cell u') - wideRank (F.cell u) ≤ w) :
    (wideData (Univ A R P K dd)).ReachesIn
      (wideRank T *
          (1 + (wideRank (F.cell gtop) + 3 + (wmRank gtop - wmRank gbot) * w +
              wideRank (F.cell gbot)) +
            (wideRank (F.cell gtop) + ((wmRank gtop - wmRank gbot) * w + 1) +
              wideRank (F.cell gbot) + 4)) + 1 +
        (wideRank (F.cell gtop) + 3 + (wmRank gtop - wmRank gbot) * w + wideRank (F.cell gbot)))
      ⟨Sum.inr (PR.stElt pChk fc), Sum.inl (fun _ => False),
        wideTape (PR.trackTapeAt F.cell t (bg fun _ => False) fun _ => False) (PR.syElt PR.blank)⟩
      ⟨Sum.inr (PR.stElt pTy fc), Sum.inl T,
        wideTape (PR.trackTapeAt F.cell t (bg T) T) (PR.syElt PR.blank)⟩ :=
  reachesIn_ixSeekTo F.toIx hR hlin hlin (elt := id) (Use := fun _ => True)
    (fun _ _ h => h) (fun _ _ => Iff.rfl) (fun _ x _ _ => ⟨x, trivial, rfl⟩)
    hnerg hnerl hnewk hnetg htop hbot (fun x _ => ⟨x, trivial, rfl⟩) hT
    hbgwk hbgNwk hbgNoth hbgrg hbgrl hbgtg
    hturn hscanT hbT₁ hbT₂ hpass hfail hheldN hstayY hbackN hA₀ hA₁
    hscanA hbA₁ hbA₂ hclear hset hholdC hstayA hbackC hgap
