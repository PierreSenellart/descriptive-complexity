...
`DescriptiveComplexity.Problems.Wide.PfpInstVar` read at a coarse file: the
...
noncomputable def ixVarGatesCost (dt : PfpData L) (A : Type) (vi : dt.VarIx)
    (w wP : ℕ) : ℕ :=
...
include hrules hR hlin hix hsepP hhasP hinj heltP hord htop hbot hwork hv hvi
  he₀ hmono hup hvh hxdUse hgap hwP hwR hwK hcostR in
...
checkpoint. -/
theorem ixVarMatrix_run (stV : TapeSt dt A R P I)
    ...
    (sem : ∀ a : Fin (dt.natOf vi),
      dt.IxKindSem PR.zero PR.one vi stV elt (dt.kindOf vi a))
    (f : dt.CtlIx → A) :
    Relation.ReflTransGen (wideData (Univ A R P dt.KIx dt.dd)).Step
      ...
      ⟨Sum.inr (PR.stElt (emb .mchk1)
          (dt.ixMatFs (elt := elt) F PR.zero PR.one hhasP vi stV v
  ...
  refine Relation.ReflTransGen.trans
    (ixMatrix_run (F := F) (hhasP := hhasP) (hsepP := hsepP) (hix := hix)
...
noncomputable def ixVarCost (dt : PfpData L) (A : Type) (vi : dt.VarIx)
    (w wP wR wK n : ℕ) : ℕ :=
...
theorem ixReadAcc_varFM
  ...
  | hstep w z hwz hnb ih =>
    ...
    have hblkc : dt.ixCarryBlk F (mV w) =
        tagBlk (argIn dt.ko c : PfpTag R P dt.KIx) := by
...
(`DescriptiveComplexity.Pfp.PfpData.ixRoundEndSt`). Whether it ran at all is
...
(`DescriptiveComplexity.Pfp.PfpData.ixRoundEndSt_eq`): every other register
...
abbrev IxScratch (_dt : PfpData L) (_A _R _P I : Type) : Type :=
  (I → Prop) × (I → Prop)
...
`DescriptiveComplexity.Pfp.PfpData.ixVarFM`. -/
...
`DescriptiveComplexity.Pfp.PfpData.ixRoundFX`, at the state the round's
...
that difference (`DescriptiveComplexity.Pfp.PfpData.ixRoundCtlT_eq_ixRoundCtl`,
`DescriptiveComplexity.Pfp.PfpData.ixRoundCtl_congr_scratch`). What the
...
`DescriptiveComplexity.Pfp.PfpData.ixVarMachine_run` with no boundary
...
ends in `DescriptiveComplexity.Pfp.PfpData.ixVarStE` at the top, which
