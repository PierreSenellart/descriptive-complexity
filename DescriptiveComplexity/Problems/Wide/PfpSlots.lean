...
## The track slots (`DescriptiveComplexity.Pfp.Slot`)
...
(`DescriptiveComplexity.Pfp.Prog.reaches_toCell`) read.

## The control slots (`DescriptiveComplexity.Pfp.Ctl`)
...
(`DescriptiveComplexity.Pfp.bitVal`).

`DescriptiveComplexity.Pfp.slotMark` is the mark itself, with one read-back
lemma per slot kind; `DescriptiveComplexity.Pfp.slotMark_name` is the one the
...
inductive Slot (ι : Type) (ko ki dd0 : ℕ) : Type
  ...
  `DescriptiveComplexity.Pfp.logicalTop` at startup: what the plain sweeps
  stop at. -/
...
def tagBlk : PfpTag R P (Fin ko ⊕ₗ Fin ki) → Option (Fin ko ⊕ Fin ki)
  | .arg i => some (ofLex i)
  | _ => none
...
theorem tagBlk_eq_none_of_least {x : Univ A R P (Fin ko ⊕ₗ Fin ki) dd}
  ...
  | .arg i =>
    exfalso
    rcases hleast (PfpTag.sym, x.2) with hlt | ⟨he, -⟩
    · rw [hg] at hlt
      exact absurd hlt (asymm (lt_arg PfpTag.sym i (fun j => fun h => nomatch h)))
    · rw [hg] at he
      exact nomatch he
...
theorem eq_of_slotMark_name
  ...
  have htag : tx = ty := by
    ...
    | .arg i, .arg i' =>
      ...
      exact congrArg PfpTag.arg (ofLex.injective.eq_iff.mp hii)
