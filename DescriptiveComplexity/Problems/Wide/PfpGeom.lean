...
The instance the reduction emits has universe `PfpTag R P K × (Fin dd → A)`, and
...
* a symbol is `(sym, pad w)` – the tracks in `w` (`PfpAlphabet`);
...
(`DescriptiveComplexity.Pfp.pad_injective`). Those are the distinctness facts the
...
noncomputable def stateElt (zero : A) (p : P) (w : Fin c → A) :
    PfpTag R P K × (Fin dd → A) :=
  (PfpTag.phase p, pad zero w)

/-- **A symbol**: the tracks in the payload (`DescriptiveComplexity.Pfp.withBit`). -/
noncomputable def symElt (zero : A) (w : Fin c → A) : PfpTag R P K × (Fin dd → A) :=
  (PfpTag.sym, pad zero w)

/-- **A transition**: the rule in the tag, the rule's data in the payload. -/
noncomputable def trElt (zero : A) (r : R) (w : Fin c → A) :
    PfpTag R P K × (Fin dd → A) :=
  (PfpTag.ctrl r, pad zero w)
...
theorem stateElt_inj (hc : c ≤ dd) {p p' : P} {w w' : Fin c → A}
  ...
  have h1 : (PfpTag.phase p : PfpTag R P K) = PfpTag.phase p' := congrArg Prod.fst h
  simpa using h1
...
theorem trElt_inj (hc : c ≤ dd) {r r' : R} {w w' : Fin c → A}
    ...
    r = r' ∧ w = w' := by
  refine ⟨?_, pad_injective hc (congrArg Prod.snd h)⟩
  have h1 : (PfpTag.ctrl r : PfpTag R P K) = PfpTag.ctrl r' := congrArg Prod.fst h
  simpa using h1
...
theorem stateElt_ne_symElt (p : P) (w w' : Fin c → A) :
  ...
  intro h
  have h1 : (PfpTag.phase p : PfpTag R P K) = PfpTag.sym := congrArg Prod.fst h
  simp at h1
...
theorem trElt_ne_stateElt (r : R) (p : P) (w w' : Fin c → A) :
  ...
  intro h
  have h1 : (PfpTag.ctrl r : PfpTag R P K) = PfpTag.phase p := congrArg Prod.fst h
  simp at h1
...
theorem trElt_ne_symElt (r : R) (w w' : Fin c → A) :
  ...
  intro h
  have h1 : (PfpTag.ctrl r : PfpTag R P K) = PfpTag.sym := congrArg Prod.fst h
  simp at h1
