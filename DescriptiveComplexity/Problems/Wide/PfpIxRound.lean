...
`DescriptiveComplexity.Problems.Wide.PfpInstRound` read at a coarse file: the
      ...
      ((wideRank (F.cell gtop) + 2 + ((ixRank F.le gtop - ixRank F.le gbot) * wG + 1) +
        ...
        ((w + 2) * Fintype.card dt.X.Tag + 2 +
          ((2 + (w + 2) * dt.domNr (dt.dspTagOf PR.zero PR.one
              (wmBlk (ixAddr elt st.val) (PfpTag.arg (toLex b) : PfpTag R P dt.KIx)))) *
...
`DescriptiveComplexity.Pfp.PfpData.ixIGs_run` with the levels counted – one
...
noncomputable def ixRoundCost (dt : PfpData L) (A : Type) (vi : dt.VarIx)
    (w wP wR wK : ℕ) : ℕ :=
...
include hrules hR hlin hix hsepP hhasP hinj heltP hgap hord htop hbot hwork hv hvi
  he₀ hmono hup hvh hxdUse hwP hwR hwK hcostR in
/-- **One round of the VAL loop, run — threaded**: as
`DescriptiveComplexity.Pfp.PfpData.ixRound_run` with no boundary discipline
assumed, so it applies at every address of the outer sweep. The tape ends
in `DescriptiveComplexity.Pfp.PfpData.ixRoundEndSt`, which is the entry state
unless the round's matrix ran and contained a stage atom. -/
