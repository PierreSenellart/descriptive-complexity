...
`DescriptiveComplexity.Pfp.IsEnc` – a block value encodes a point of the
...
* **every member is well-shaped** – a witness tuple or a member tuple: one
  ...
  (`DescriptiveComplexity.Pfp.PtCode`);
* **the domain sentence holds of the decoded assignment** – where
  `DescriptiveComplexity.Pfp.decRho` reads the assignment back one membership
  question per bit, which is the element-loop sub-evaluation.

`DescriptiveComplexity.Pfp.isEnc_iff_parts` is the equivalence. Uniqueness of
