...
guarded by `DescriptiveComplexity.Pfp.PfpData.exitG` – at the marker, which is
nobody's register – and its trips are named by
`DescriptiveComplexity.Pfp.PfpData.nameG` – this cell is the canonically padded
...
(`DescriptiveComplexity.Pfp.PfpData.nameGF`), the caller owing the
