...
`DescriptiveComplexity.Problems.Wide.PfpInstEval` read at a coarse file: one
...
`DescriptiveComplexity.Problems.Wide.PfpIxSpine` and
...
variable (hwork : ∀ {r : Univ A R P dt.KIx dt.dd → Prop},
  (∀ x, r x → ∃ i : dt.KIx, x.1 = PfpTag.arg i) →
...
`DescriptiveComplexity.Pfp.PfpData.ixVarMachine_run`'s final control, at the
position's variable. -/
...
control — the `ixVarFM` iteration the round hypothesis is stated over. -/
noncomputable def ixLegFM (j : Fin dt.nv)
    (st : TapeSt dt A R P I)
    (tOf : Fin (dt.arOf (dt.varAt j)) → dt.X.Tag)
    (semOf : ∀ a : ιV,
      (∀ ℓ : Fin (dt.nIn (dt.varAt j)),
...
`DescriptiveComplexity.Pfp.PfpData.ixLegCtl`, with the VAL loop's rounds run
...
is the unthreaded loop (`ixVarFMT_eq_varFM`), its last round the unthreaded
...
(`DescriptiveComplexity.Pfp.PfpData.ixLegBitB`) the verdict
    ...
    (sem₀ : ∀ a : ιV,
      ...
      ∀ b : Fin (dt.natOf (dt.varAt j)),
        dt.IxKindSem PR.zero PR.one (dt.varAt j) (dt.ixRoundSt st (mV a))
          elt (dt.kindOf (dt.varAt j) b))
    (f₀ : dt.CtlIx → A) :
    dt.ixLegCtlT (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV j st tOf
        (dt.ixSemCastT F (dt.varAt j) st v mV sem₀) f₀ =
      dt.ixLegCtl (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV j st tOf sem₀ f₀ := by
  have hSE : dt.ScratchEq
      (dt.ixVarStE (elt := elt) F hinj hhasP heltP
        (dt.varAt j) st v mV (dt.ixSemCastT F (dt.varAt j) st v mV sem₀)
        (dt.ixVarFG (PR := PR) F hhasP (dt.varAt j) st v tOf f₀) aT)
      (dt.ixRoundSt st (mV aT)) :=
    (dt.ixScratchEq_roundEndSt (elt := elt) (zero := PR.zero) (one := PR.one)
      (hhas := hhasP) F (dt.varAt j) _ v _).trans
      (dt.ixScratchEq_varRdSt st _ (mV aT))
  rw [ixLegCtlT, ixLegCtl,
    dt.ixVarFXT_eq_roundFX (elt := elt) F hinj hhasP heltP (dt.varAt j) st mV sem₀ hreg
      (dt.ixVarFG (PR := PR) F hhasP (dt.varAt j) st v tOf f₀) aT,
    dt.ixVarFMT_eq_varFM (elt := elt) F hinj hhasP heltP (dt.varAt j) st mV sem₀ hreg
      (dt.ixVarFG (PR := PR) F hhasP (dt.varAt j) st v tOf f₀) aT, hSE.ixBack hreg]
  rfl
...
single width. -/
noncomputable def ixLegCost (dt : PfpData L) (A : Type) (w wP wR wK n : ℕ) : ℕ :=
...
noncomputable def ixOutLegCost (dt : PfpData L) (A : Type) (w wP wR wK n : ℕ) : ℕ :=
...
per VAL round, added. -/
noncomputable def ixVarCD (dt : PfpData L) (A : Type) (vi : dt.VarIx)
    (w wP wR wK : ℕ) : ℕ :=
...
own two steps. -/
noncomputable def ixLegWidth (dt : PfpData L) (A : Type) (w wP wR wK : ℕ) : ℕ :=
...
noncomputable def ixEvalWidth (dt : PfpData L) (A : Type) (w wP wR wK : ℕ) : ℕ :=
...
`DescriptiveComplexity.Pfp.PfpData.nexProg_wideAccept_legs` asks of the
...
theorem ixVarLeg_reachesIn (j : Fin dt.nv)
    (st : TapeSt dt A R P I)
    ...
    (wideData (Univ A R P dt.KIx
      ...
      ⟨Sum.inr (PR.stElt (ep (.chk j.succ))
          (dt.ixLegCtl (elt := elt)
            (v := v) (aT := aT) F hinj hhasP heltP mV j st tOf semOf f₀)), Sum.inl v',
        wideTape (PR.trackTapeAt F.cell Slot.val
          (dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le
            (dt.ixPostVarSt v st (mV aT) (dt.varList.get j)
              ((dt.varArgsOf PR.zero PR.one (dt.varAt j)).accBit
                (dt.ixLegCtl (elt := elt)
                  (v := v) (aT := aT) F hinj hhasP heltP mV j st tOf semOf f₀))))
  ...
  have hmach := ixVarMachine_reachesIn (F := F) (hhasP := hhasP) (hsepP := hsepP)
    ...
    (hord := hord) (vi := (dt.varAt j)) (st := st) (hrules := hrulesJ) (hR := hR)
  ...
  have hexit := dt.step_var_exit F hrulesV hR hlin hix hbot hv hvi
    (f := dt.ixLegCtl (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV j st tOf semOf f₀)
    (dt.ixVarBg_back F hix htop (dt.ixRoundSt st (mV aT)) hwkSt) hns
    (fun r hr => dt.ixBack_postVarSt_off (zero := PR.zero) (one := PR.one)
      (st := dt.ixRoundSt st (mV aT)) (m := mV aT) (i := dt.varList.get j)
      (b := _) F v r hr)
    (dt.ixBack_postVarSt_v (zero := PR.zero) (one := PR.one) F v)
...
`DescriptiveComplexity.Pfp.PfpData.ixVarLeg_run` without the boundary
hypotheses `hsav`/`htgt`, which the sweep cannot supply at more than one
address. The leg ends in
`DescriptiveComplexity.Pfp.PfpData.ixLegStT`, the machinery's own exit state
...
`DescriptiveComplexity.Pfp.PfpData.ixVarLeg_run` and
`DescriptiveComplexity.Pfp.PfpData.ixVarLegFail_run` this covers **every**
...
`DescriptiveComplexity.Pfp.PfpData.ixLegCtlT_eq_legCtl` at the output
    ...
    (sem₀ : ∀ a : ιV,
      ...
      ∀ b : Fin (dt.natOf (none : dt.VarIx)),
        dt.IxKindSem PR.zero PR.one none (dt.ixRoundSt st (mV a))
          elt (dt.kindOf none b))
    (f₀ : dt.CtlIx → A) :
    dt.ixOutCtlT (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf
        (dt.ixSemCastT F none st v mV sem₀) f₀ =
      dt.ixOutCtl (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf sem₀ f₀ := by
  have hSE : dt.ScratchEq
      (dt.ixVarStE (elt := elt) F hinj hhasP heltP none st v mV (dt.ixSemCastT F none st v mV sem₀)
        (dt.ixVarFG (PR := PR) F hhasP none st v tOf f₀) aT)
      (dt.ixRoundSt st (mV aT)) :=
    (dt.ixScratchEq_roundEndSt (elt := elt) (zero := PR.zero) (one := PR.one)
      (hhas := hhasP) F none _ v _).trans
      (dt.ixScratchEq_varRdSt st _ (mV aT))
  rw [ixOutCtlT, ixOutStE, ixOutCtl,
    dt.ixVarFXT_eq_roundFX (elt := elt) F hinj hhasP heltP none st mV sem₀ hreg
      (dt.ixVarFG (PR := PR) F hhasP none st v tOf f₀) aT,
    dt.ixVarFMT_eq_varFM (elt := elt) F hinj hhasP heltP none st mV sem₀ hreg
      (dt.ixVarFG (PR := PR) F hhasP none st v tOf f₀) aT, hSE.ixBack hreg]
  rfl
...
`DescriptiveComplexity.Pfp.PfpData.ixOutLeg_run` without the boundary
...
the marker (`DescriptiveComplexity.Pfp.PfpData.ixOutStA`), and with it whether
...
theorem ixOutLeg_run_verdict
    ...
    (wideData (Univ A R P dt.KIx
      ...
      ⟨Sum.inr (PR.stElt (ep (.sub (dt.smEntryOut))) f₀), Sum.inl v',
        wideTape (PR.trackTapeAt F.cell Slot.val
          (dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le st) st.val) (PR.syElt PR.blank)⟩
      ⟨Sum.inr (PR.stElt accPh
          (dt.ixOutCtlT (elt := elt)
            (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀)), Sum.inl v',
        wideTape (PR.trackTapeAt F.cell Slot.val
          (dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le
            (dt.ixOutStA (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀))
          (mV aT)) (PR.syElt PR.blank)⟩ := by
  classical
  have hzo := PR.zero_ne_one
  set embO : dt.VarPhF (none : dt.VarIx) → P :=
  ...
  have hmach := ixVarMachine_run_thread_reachesIn (F := F) (hhasP := hhasP) (hsepP := hsepP)
    (hix := hix) (hinj := hinj) (heltP := heltP) (he₀ := he₀) (hord := hord)
    (vi := none) (st := st) (hrules := hrulesJ) (hR := hR) (hlin := hlin)
    (htop := htop) (hbot := hbot) (hwork := hwork) (hv := hv) (hvi := hvi)
    (hwkSt := hwkSt) (hmono := hmono) (hup := hup) (hvh := hvh)
    (hxdUse := hxdUse) (hgap := hgap) (hwP := hwP) (hwR := hwR) (hwK := hwK)
    (hcostR := hcostR) (TestOf := TestOf) (hcompatOf := hcompatOf)
    ...
    (hTestOf := hTestOf) (f₀ := f₀)
  -- the loop's exit state, and the register it holds
  set stE := dt.ixOutStE (elt := elt)
    (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀ with hstE
  have hvalE : stE.val = mV aT := by
    rw [hstE]; exact dt.ixVarStE_val (elt := elt) F hinj hhasP heltP none st mV semT _ aT
  have hwkE : stE.wk = fun r => r = v := by
    rw [hstE, ixOutStE, ixVarStE]
    refine ((dt.ixRoundEndSt_fields (elt := elt) (zero := PR.zero)
      (one := PR.one) (hhas := hhasP) F none _ v _).1).trans ?_
    exact ((dt.ixVarStT_fields (elt := elt) F hinj hhasP heltP none st mV semT
      (dt.ixVarFG (PR := PR) F hhasP none st v tOf f₀) aT).1).trans hwkSt
  have hround : dt.ixRoundSt stE (mV aT) = stE := hvalE ▸ dt.ixRoundSt_val stE
  -- the exit state: the same tape with the marker rewritten by the verdict
  have hA : dt.ixOutStA (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀ =
      { stE with wk := (fun r => r = v ∧
        (dt.varArgsOf PR.zero PR.one none).accBit
          (dt.ixOutCtlT (elt := elt)
            (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀)) } := by
    rw [hstE]; rfl
  have hslot : ∀ (X : TapeSt dt A R P I)
      (w : (Univ A R P dt.KIx dt.dd → Prop) →
        Prop)
      (r : Univ A R P dt.KIx dt.dd → Prop)
      (sl : dt.SlotIx), sl ≠ Slot.wk →
      dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le { X with wk := w } r sl =
        dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le X r sl := by
    intro X w r sl hsl
    cases sl <;> first
  ...
  have hoff : ∀ r, r ≠ v →
      dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le
        (dt.ixOutStA (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀) r =
      dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le stE r := by
    intro r hr
    rw [hA]
    funext sl
    by_cases hsl : sl = Slot.wk
    · subst hsl
      rw [ixBack_wk, ixBack_wk, hwkE]
      exact bitVal_congr (iff_of_false (fun hc => hr hc.1) hr)
    · exact hslot _ _ r sl hsl
  have hupd : dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le
        (dt.ixOutStA (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀) v =
      Function.update (dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le stE v)
        ((dt.varArgsOf PR.zero PR.one none).newSlot)
        (bitVal PR.zero PR.one
          ((dt.varArgsOf PR.zero PR.one none).accBit
            (dt.ixOutCtlT (elt := elt)
  ...
  have hns : (dt.varArgsOf PR.zero PR.one (none : dt.VarIx)).newSlot ≠
      Slot.val := fun h => nomatch h
  have hexit := dt.step_var_exit F hrulesV hR hlin hix hbot hv hvi
    (f := dt.ixOutCtlT (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀)
    (rest := dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le stE)
    (rest' := dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le
      (dt.ixOutStA (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀))
    (mval := mV aT)
...
`DescriptiveComplexity.Pfp.PfpData.ixOutLeg_run_verdict` in which the accepting
...
theorem ixOutLeg_run_thread
    ...
    (wideData (Univ A R P dt.KIx
      ...
      ⟨Sum.inr (PR.stElt accPh
        ...
        wideTape (PR.trackTapeAt F.cell Slot.val
          (dt.ixBack F.toLayout PR.zero PR.one dt.dd0Le
            (dt.ixOutStE (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀))
          (mV aT)) (PR.syElt PR.blank)⟩ := by
  have hwkE : (dt.ixOutStE (elt := elt)
    (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀).wk =
      fun r => r = v := by
    rw [ixOutStE, ixVarStE]
    refine ((dt.ixRoundEndSt_fields (elt := elt) (zero := PR.zero)
      (one := PR.one) (hhas := hhasP) F none _ v _).1).trans ?_
    exact ((dt.ixVarStT_fields (elt := elt) F hinj hhasP heltP none st mV semT
      (dt.ixVarFG (PR := PR) F hhasP none st v tOf f₀) aT).1).trans hwkSt
  have heq : dt.ixOutStA (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀ =
      dt.ixOutStE (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀ := by
    have hw : (fun r => r = v ∧ (dt.varArgsOf PR.zero PR.one none).accBit
        (dt.ixOutCtlT (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀)) =
        (dt.ixOutStE (elt := elt) (v := v) (aT := aT) F hinj hhasP heltP mV st tOf semT f₀).wk := by
      rw [hwkE]
      exact funext fun r => propext (and_iff_left hacc)
    simp only [ixOutStA, hw]
  exact heq ▸ ixOutLeg_run_verdict (F := F) (hhasP := hhasP) (hsepP := hsepP)
    (hix := hix) (hinj := hinj) (heltP := heltP) (he₀ := he₀) (hord := hord)
    (hrulesOut := hrulesOut) (hR := hR) (hlin := hlin) (htop := htop) (hbot := hbot)
    (hwork := hwork) (hv := hv) (hvi := hvi) (hmono := hmono) (hup := hup)
    (hvh := hvh) (hxdUse := hxdUse) (hgap := hgap) (hwP := hwP) (hwR := hwR)
    (hwK := hwK) (hcostR := hcostR) (hbotV := hbotV) (htopV := htopV) (mV := mV)
    (hmV0 := hmV0) (hIncr := hIncr) (hTestT := hTestT) (hTestF := hTestF)
    (st := st) (hwkSt := hwkSt) (TestOf := TestOf) (hcompatOf := hcompatOf)
    (tOf := tOf) (hwitOf := hwitOf) (hmir := hmir) (hbotSt := hbotSt)
    (semT := semT) (hDom := hDom) (hTestOf := hTestOf) (f₀ := f₀)

/-! ### Which leg a position takes

A position's machinery has three runs, by what its gates do:
`DescriptiveComplexity.Pfp.PfpData.ixVarLeg_run_thread` when every block is
well shaped *and* the tags and the domain sentence agree,
`DescriptiveComplexity.Pfp.PfpData.ixVarLegUngated_run` when the blocks are
well shaped but the verdict flag is cleared, and
`DescriptiveComplexity.Pfp.PfpData.ixVarLegFail_run` when a block fails the
...
`DescriptiveComplexity.Pfp.PfpData.ixShapeAt` this is the gates' verdict. -/
...
gated one is `DescriptiveComplexity.Pfp.PfpData.ixLegStT_fields`. The `val`
...
(`DescriptiveComplexity.Pfp.PfpData.ixVarStE_new`) and the ungated legs
