...
The universe a `PfpData` reduction draws is `PfpTag R P K × (Fin dd → A)`, and a
**logical** address – one whose non-argument blocks are empty
(`DescriptiveComplexity.Pfp.logicalTop`) – is exactly an address of the working
...
in (`DescriptiveComplexity.Pfp.PfpData.nexTotal_lt_two_pow'`).
-/
...
theorem card_argTag :
    Nat.card {τ : PfpTag R P K // ∃ i : K, τ = PfpTag.arg i} = Nat.card K :=
  Nat.card_congr
    { toFun := fun τ => τ.2.choose
      invFun := fun i => ⟨PfpTag.arg i, i, rfl⟩
      left_inv := by
        rintro ⟨τ, hτ⟩
        exact Subtype.ext hτ.choose_spec.symm
      right_inv := by
        intro i
        exact PfpTag.arg.inj (Exists.choose_spec (⟨i, rfl⟩ :
          ∃ i' : K, (PfpTag.arg i : PfpTag R P K) = PfpTag.arg i')).symm }
...
theorem card_pfpTag :
    Nat.card (PfpTag R P K) = Nat.card R + 1 + Nat.card P + Nat.card K := by
  classical
  have e : PfpTag R P K ≃ R ⊕ (Unit ⊕ (P ⊕ K)) :=
...
theorem card_univ_eq :
  ...
  rw [Univ, Nat.card_prod, card_pfpTag]
