...
(`DescriptiveComplexity.Problems.Wide.PfpRepAtoms`).

This file is the semantic anchor of that loop, the control-scale twin of
`DescriptiveComplexity.Problems.Wide.PfpLeaf`:
`DescriptiveComplexity.Pfp.PfpData.relMap_iff_altQuantFrom_expLeaf` says the
expanded relation **is** the prefix of
`DescriptiveComplexity.Pfp.PfpData.expLeaf` played from level `0`, and the
...
(`DescriptiveComplexity.Pfp.altQuantFrom_pad`).
-/
...
variable {L : Language.{0, 0}} [L.IsRelational] (dt : PfpData L) {A : Type}
