...
`DescriptiveComplexity.Pfp.PfpData.evalSpine_run` takes the per-position
...
* **the marker's `new` cell of a variable is that variable's verdict**
  ...
  `DescriptiveComplexity.Pfp.PfpData.varList` being `Finset.univ.toList`,
  hence duplicate-free and complete (`exists_varList_get`).

Joined with `DescriptiveComplexity.Pfp.PfpData.accVerdict_next`, that is
...
`DescriptiveComplexity.Pfp.PfpData.evalSpine_run` asks for.
...
obligations of `DescriptiveComplexity.Pfp.PfpData.reaches_sweep`, and
...
`DescriptiveComplexity.Pfp.PfpData.reaches_sweep`'s `hSW` and `hFS`.
...
the address order, which is `DescriptiveComplexity.Pfp.iterOrd` at the
...
theorem new_postVarSt (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
...
variable {stOf : Fin (dt.nv + 1) →
  TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF))}
...
is not literally a
`DescriptiveComplexity.Pfp.PfpData.postVarSt` of the position's entry
state (its VAL loop may normalize the two scratch registers first), while
this projection of it is
(`DescriptiveComplexity.Pfp.PfpData.legStB_new`). -/
abbrev WritesNew
    (stOf : Fin (dt.nv + 1) → TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
...
theorem spineRide {β : Sort _}
    (F : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) → β)
...
(`DescriptiveComplexity.Pfp.PfpData.varLegFail_run`,
`DescriptiveComplexity.Pfp.PfpData.varLegUngated_run`) store `False`, which
...
theorem new_last_next_at
    ...
    (hKin : ∀ (a : ιV)
      (t : PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx)
    ...
    (semOfJ : ∀ a : ιV,
      (∀ ℓ : Fin (dt.nIn (dt.varAt j)),
        dt.igPassP PR.zero PR.one (dt.varAt j)
    ...
    (hbj : bOf j ↔
      dt.accVerdict PR.one (dt.polOf (dt.varAt j))
        ((dt.varArgsOf PR.zero PR.one (dt.varAt j)).postFold
          (dt.roundFX (dt.varAt j) (stOf j.castSucc) v mV semOfJ
            (dt.varFM (dt.varAt j) (stOf j.castSucc) v mV semOfJ fG aT) aT)
    ...
    (hsem : ∀ (a : ιV)
      ...
      (hmb' : ∀ ℓ : Fin (dt.arOf (dt.varAt j)),
        wmBlk (dt.roundSt (stOf j.castSucc) (mV a)).mir
          ...
          encMap dt.ly PR.zero PR.one (mbW ℓ)),
      semOfJ a hp b = dt.passSem PR.zero_ne_one hlin (dt.varAt j)
  ...
  exact dt.accVerdict_next (stOf j.castSucc) mV (dt.varList.get j)
...
`DescriptiveComplexity.Pfp.PfpData.accVerdict_next` reads as
...
theorem new_last_next
    ...
    (hbOf : ∀ j : Fin dt.nv, bOf j ↔
      dt.accVerdict PR.one (dt.polOf (dt.varAt j))
        ((dt.varArgsOf PR.zero PR.one (dt.varAt j)).postFold
          (dt.roundFX (dt.varAt j) (stOf j.castSucc) v mV (semOfJ j)
            (dt.varFM (dt.varAt j) (stOf j.castSucc) v mV (semOfJ j)
              (fGOf j) aT) aT)
          (dt.back PR.zero PR.one dt.dd0Le
            (dt.roundSt (stOf j.castSucc) (mV aT)) v)))
    ...
    (hsem : ∀ (j : Fin dt.nv) (a : ιV)
      ...
      semOfJ j a hp b = dt.passSem PR.zero_ne_one hlin (dt.varAt j)
  ...
  obtain ⟨j, hj⟩ := dt.exists_varList_get i
  subst hj
  exact dt.new_last_next_at mV hlin hord hbotV hmV0 hIncr hKin hTop σ j
...
`hSW` of `DescriptiveComplexity.Pfp.PfpData.reaches_sweep`).
...
theorem sweep_new
    ...
    {SW stE : (Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd →
      Prop) → TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF))}
    {N : dt.d.B.ι →
...
(`DescriptiveComplexity.Pfp.not_trackOf_of_notEnc`), so the two readings
agree there too. -/
...
`DescriptiveComplexity.Pfp.trackOf_of_blocks`, the sweep's mirror
...
theorem new_last_trackOf
    ...
    ((stOf (Fin.last dt.nv)).new i v ↔
      trackOf dt.ly PR.zero PR.one (dt.arOf_le_ko (some i))
        (dt.d.next σ) v) := by
  refine (dt.new_last_next mV hlin hord hbotV hmV0 hIncr hKin hTop σ semOfJ
  ...
  change wmBlk (stOf 0).mir
    (PfpTag.arg (toLex (Sum.inl k : Fin dt.ko ⊕ Fin dt.ki)) :
      PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx) = _
  rw [hmir]
  exact hpt k
...
omit [Fintype dt.SlotIx] [L.IsRelational] [Finite dt.KIx]
...
theorem passW_congr {zero one : A} (hzo : zero ≠ one)
    (hlin : IsLinOrd (WMLe (A := Univ A R (OuterPh (EvalPh dt.nv dt.PMF))
      dt.KIx dt.dd)))
    (vi : dt.VarIx)
    {st st' : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF))}
    (hmir : st.mir = st'.mir) (hval : st.val = st'.val)
    (hp : ∀ ℓ : Fin (dt.nIn vi), dt.igPassP zero one vi st ℓ)
    (hp' : ∀ ℓ : Fin (dt.nIn vi), dt.igPassP zero one vi st' ℓ)
    ...
    (hmb' : ∀ ℓ : Fin (dt.arOf vi),
      wmBlk st'.mir
        (PfpTag.arg (toLex ((Sum.inl (Fin.castLE (dt.arOf_le_ko vi) ℓ) :
          ...
          PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx) =
        encMap dt.ly zero one (mbW ℓ)) :
    dt.passW zero one hzo hlin vi st hp mbW =
      dt.passW zero one hzo hlin vi st' hp' mbW := by
  funext j
  refine encMap_injective dt.ly hzo ?_
  rw [← dt.passW_hENC hzo hlin vi st hp mbW hmb j,
    ← dt.passW_hENC hzo hlin vi st' hp' mbW hmb' j,
...
theorem kindSemCast_passSem {zero one : A} (hzo : zero ≠ one)
    ...
    dt.kindSemCast zero one vi hmir hval (dt.kindOf vi b)
        (dt.passSem hzo hlin vi st hp mbW hmb b) =
      dt.passSem hzo hlin vi st' hp' mbW hmb' b :=
  dt.kindSemCast_mkKindSem zero one vi hmir hval
    (dt.passW_congr hzo hlin vi hmir hval hp hp' mbW hmb hmb')
    (dt.passW_hENC hzo hlin vi st hp mbW hmb)
    (dt.passW_hENC hzo hlin vi st' hp' mbW hmb') (dt.kindOf vi b)
...
noncomputable def spineSem (zero one : A) (vi : dt.VarIx)
    ...
    (sem₀ : ∀ a : ιV,
      (∀ ℓ : Fin (dt.nIn vi),
        dt.igPassP zero one vi (dt.roundSt st₀ (mV a)) ℓ) →
      ∀ b : Fin (dt.natOf vi),
  ...
  dt.kindSemCast zero one vi (st := dt.roundSt st₀ (mV a))
    ...
    (sem₀ a (fun ℓ =>
      (dt.igPassP_roundSt zero one vi st st₀ (mV a) ℓ).mp (hp ℓ)) b)

/-- **One pack, carried to every round of every position — threaded**: as
`DescriptiveComplexity.Pfp.PfpData.spineSem`, at the states the VAL loop's
...
noncomputable def spineSemT (zero one : A) (vi : dt.VarIx)
    ...
    (hp : ∀ ℓ : Fin (dt.nIn vi),
      dt.igPassP zero one vi (dt.varRdSt st p (mV a)) ℓ)
    (b : Fin (dt.natOf vi)) :
...
theorem spineSem_passSem {zero one : A} (hzo : zero ≠ one)
    ...
    (hsem₀ : ∀ (a : ιV)
      ...
      (hm₀ : ∀ ℓ : Fin (dt.arOf vi),
        wmBlk (dt.roundSt st₀ (mV a)).mir
          ...
          encMap dt.ly zero one (mbW ℓ)),
      sem₀ a hp₀ b =
        dt.passSem hzo hlin vi (dt.roundSt st₀ (mV a)) hp₀ mbW hm₀ b)
    (a : ιV)
    (hp : ∀ ℓ : Fin (dt.nIn vi),
      dt.igPassP zero one vi (dt.roundSt st (mV a)) ℓ)
    (b : Fin (dt.natOf vi))
    (hm : ∀ ℓ : Fin (dt.arOf vi),
      wmBlk (dt.roundSt st (mV a)).mir
        (PfpTag.arg (toLex ((Sum.inl (Fin.castLE (dt.arOf_le_ko vi) ℓ) :
          ...
          PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx) =
        encMap dt.ly zero one (mbW ℓ)) :
    dt.spineSem zero one vi mV sem₀ hmir a hp b =
      dt.passSem hzo hlin vi (dt.roundSt st (mV a)) hp mbW hm b := by
  ...
  refine Eq.trans (congrArg
    (dt.kindSemCast zero one vi (st := dt.roundSt st₀ (mV a))
      (st' := dt.roundSt st (mV a)) hmir.symm rfl (dt.kindOf vi b))
    (hsem₀ a _ b hm₀)) ?_
  exact dt.kindSemCast_passSem hzo hlin vi (st := dt.roundSt st₀ (mV a))
...
The branch `DescriptiveComplexity.Pfp.PfpData.gatedAt` takes is not merely
...
exist at all. `DescriptiveComplexity.Pfp.PfpData.gate_trichotomy` says the
three legs are exhaustive; read in the other direction it says a gated
position's blocks encode points. -/

omit [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [L.IsRelational] in
    ...
    IsEnc dt.ly PR.zero PR.one
      (wmBlk st.mir
        (PfpTag.arg (toLex ((Sum.inl
          (Fin.castLE (dt.arOf_le_ko (dt.varAt j)) ℓ) :
          Fin dt.ko ⊕ Fin dt.ki))) :
          PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx)) := by
  classical
  have hpt : ∀ r, dt.back PR.zero PR.one dt.dd0Le st r Slot.mir =
      bitVal PR.zero PR.one (regBit st.mir r) := fun _ => rfl
  rcases dt.gate_trichotomy hzo hlin st
    (Sum.inl (Fin.castLE (dt.arOf_le_ko (dt.varAt j)) ℓ)) with
    h | ⟨u₀, hbad⟩ | ⟨-, hbad⟩
  · exact h
  · refine absurd ?_ hbad
    have h := hg.1 ℓ u₀
    rwa [shapeAt, passTracks_of_back hpt (wmSeg u₀)] at h
...
`DescriptiveComplexity.Pfp.PfpData.testOf_of_encMap`,
      ...
      IsEnc dt.ly PR.zero PR.one
        (wmBlk st.mir
          (PfpTag.arg (toLex ((Sum.inl
            ...
            Fin dt.ko ⊕ Fin dt.ki))) :
            PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx))) :
    dt.gatedAt (PR := PR) j st := by
  classical
  have hpt : ∀ r, dt.back PR.zero PR.one dt.dd0Le st r Slot.mir =
      bitVal PR.zero PR.one (regBit st.mir r) := fun _ => rfl
  refine ⟨fun ℓ u => ?_, fun ℓ => ?_⟩
  · obtain ⟨p, hp⟩ := henc ℓ
    rw [shapeAt, passTracks_of_back hpt (wmSeg u)]
    exact dt.testOf_of_encMap hzo hlin hp u
  · obtain ⟨p, hp⟩ := henc ℓ
...
noncomputable def gatedSem (hzo : PR.zero ≠ PR.one)
  ...
  dt.kindSemCast PR.zero PR.one (dt.varAt j)
    ...
    ((dt.matSt_fields (v := v)
      (st := dt.varRdSt st p (mV a)) (b : ℕ)).2.2.2.1).symm
    (dt.kindOf (dt.varAt j) b)
    (dt.passSem hzo hlin (dt.varAt j) (dt.varRdSt st p (mV a)) hp
      (fun ℓ => (dt.isEnc_of_gatedAt hzo hlin j st hg ℓ).choose)
      (fun ℓ => (dt.isEnc_of_gatedAt hzo hlin j st hg ℓ).choose_spec) b)
...
noncomputable def gatedSem₀ (hzo : PR.zero ≠ PR.one)
    ...
    {ιV : Type} (mV : ιV →
      Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (j : Fin dt.nv) (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (hg : dt.gatedAt (PR := PR) j st) (a : ιV)
    (hp : ∀ ℓ : Fin (dt.nIn (dt.varAt j)),
      dt.igPassP PR.zero PR.one (dt.varAt j) (dt.roundSt st (mV a)) ℓ)
    (b : Fin (dt.natOf (dt.varAt j))) :
    dt.KindSem PR.zero PR.one (dt.varAt j) (dt.roundSt st (mV a))
      (dt.kindOf (dt.varAt j) b) :=
  dt.passSem hzo hlin (dt.varAt j) (dt.roundSt st (mV a)) hp
    (fun ℓ => (dt.isEnc_of_gatedAt hzo hlin j st hg ℓ).choose)
    (fun ℓ => (dt.isEnc_of_gatedAt hzo hlin j st hg ℓ).choose_spec) b

omit [L.IsRelational] [Finite dt.KIx] in
...
`DescriptiveComplexity.Pfp.PfpData.gatedSem₀` — the hypothesis the VAL
...
theorem gatedSem_eq_semCastT (hzo : PR.zero ≠ PR.one)
    ...
    (j : Fin dt.nv) (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (hg : dt.gatedAt (PR := PR) j st) :
    dt.gatedSem hzo hlin mV (v := v) j st hg =
      dt.semCastT (dt.varAt j) st v mV (dt.gatedSem₀ hzo hlin mV j st hg) := by
  funext p a hp b
  ...
  have hpA : ∀ ℓ : Fin (dt.nIn (dt.varAt j)),
    ...
    fun ℓ => (dt.igPassP_congr PR.zero PR.one (dt.varAt j) hval ℓ).mp (hp ℓ)
  have hmbA : ∀ ℓ : Fin (dt.arOf (dt.varAt j)),
      wmBlk (dt.matSt (dt.varAt j) (dt.varRdSt st p (mV a)) v (b : ℕ)).mir
        (PfpTag.arg (toLex ((Sum.inl (Fin.castLE (dt.arOf_le_ko (dt.varAt j)) ℓ) :
          Fin dt.ko ⊕ Fin dt.ki))) :
          PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx) =
        encMap dt.ly PR.zero PR.one
          ((dt.isEnc_of_gatedAt hzo hlin j st hg ℓ).choose) := by
    intro ℓ
    rw [(dt.matSt_fields (v := v) (st := dt.varRdSt st p (mV a)) (b : ℕ)).2.1]
    exact (dt.isEnc_of_gatedAt hzo hlin j st hg ℓ).choose_spec
  exact (dt.kindSemCast_passSem hzo hlin (dt.varAt j) _ _ hp hpA _ _ hmbA b).trans
    (dt.kindSemCast_passSem hzo hlin (dt.varAt j) _ _ _ hpA _ _ hmbA b).symm
...
theorem legBitB_gatedSem (hzo : PR.zero ≠ PR.one)
    ...
    (mV : ιV → Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (j : Fin dt.nv) (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (hg : dt.gatedAt (PR := PR) j st) (f₀ : dt.CtlIx → A) :
    dt.legBitB (v := v) (aT := aT) mV j st
        (fun hg' => dt.gatedSem hzo hlin mV (v := v) j st hg') f₀ =
      (dt.varArgsOf PR.zero PR.one (dt.varAt j)).accBit
        (dt.legCtl (v := v) (aT := aT) mV j st (dt.tagAt (PR := PR) j st)
          (dt.gatedSem₀ hzo hlin mV j st hg) f₀) := by
  classical
  rw [legBitB, dif_pos hg, dt.gatedSem_eq_semCastT hzo hlin mV j st hg,
    dt.legCtlT_eq_legCtl mV hreg j st (dt.tagAt (PR := PR) j st) _ f₀]
...
variable (mV : ιV → Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd →
  Prop)
variable (st₀ : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
...
noncomputable def spineNode :
    ℕ → Σ' st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)),
      PProd (st.mir = st₀.mir) (dt.CtlIx → A)
  | 0 => ⟨st₀, rfl, f₀⟩
  | n + 1 =>
    let prev := spineNode n
    if h : n < dt.nv then
      let f' := dt.legCtl (v := v) (aT := aT) mV ⟨n, h⟩ prev.1 (tOf ⟨n, h⟩)
        (dt.spineSem PR.zero PR.one (dt.varAt ⟨n, h⟩) mV (sem₀ ⟨n, h⟩)
          prev.2.1) prev.2.2
...
noncomputable def spineStOf (k : Fin (dt.nv + 1)) :
    TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) :=
  (dt.spineNode (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf (k : ℕ)).1

/-- The control family of the spine. -/
noncomputable def spineFsOf (k : Fin (dt.nv + 1)) : dt.CtlIx → A :=
  (dt.spineNode (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf (k : ℕ)).2.2
...
theorem spineStOf_mir (k : Fin (dt.nv + 1)) :
    (dt.spineStOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf k).mir = st₀.mir :=
  (dt.spineNode (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf (k : ℕ)).2.1

/-- The packs of the built family: the entry state's, carried. -/
noncomputable def spineSemOf (j : Fin dt.nv) (a : ιV)
    (hp : ∀ ℓ : Fin (dt.nIn (dt.varAt j)),
      dt.igPassP PR.zero PR.one (dt.varAt j)
        (dt.roundSt (dt.spineStOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
          j.castSucc) (mV a)) ℓ)
    (b : Fin (dt.natOf (dt.varAt j))) :
    dt.KindSem PR.zero PR.one (dt.varAt j)
      (dt.roundSt (dt.spineStOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
        j.castSucc) (mV a)) (dt.kindOf (dt.varAt j) b) :=
  dt.spineSem PR.zero PR.one (dt.varAt j) mV (sem₀ j)
    (dt.spineStOf_mir (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc)
    a hp b

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem spineStOf_zero :
    dt.spineStOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf 0 = st₀ := rfl

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem spineFsOf_zero :
    dt.spineFsOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf 0 = f₀ := rfl

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
/-- **The control's cover equation** — `evalSpine_run`'s `hfs`. -/
theorem spineFsOf_succ (j : Fin dt.nv) :
    dt.spineFsOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.succ =
      dt.legCtl (v := v) (aT := aT) mV j
        (dt.spineStOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc)
        (tOf j)
        (dt.spineSemOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j)
        (dt.spineFsOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc) := by
  change (dt.spineNode (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
    ((j : ℕ) + 1)).2.2 = _
...
theorem spineStOf_succ (j : Fin dt.nv) :
    dt.spineStOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.succ =
      dt.postVarSt v
        (dt.spineStOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc)
        ...
        ((dt.varArgsOf PR.zero PR.one (dt.varAt j)).accBit
          (dt.legCtl (v := v) (aT := aT) mV j
            ...
            (tOf j)
            (dt.spineSemOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j)
            (dt.spineFsOf (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
              j.castSucc))) := by
  change (dt.spineNode (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
    ((j : ℕ) + 1)).1 = _
  rw [spineNode, dif_pos j.isLt]
...
`DescriptiveComplexity.Pfp.PfpData.spineNode`, with the leg's own exit
...
noncomputable def spineNodeT :
  ...
  | n + 1 =>
    ...
    if h : n < dt.nv then
      ⟨dt.legStT (v := v) (aT := aT) mV ⟨n, h⟩ prev.1 (tOf ⟨n, h⟩)
          (dt.spineSemT PR.zero PR.one (dt.varAt ⟨n, h⟩) (v := v) mV
            (sem₀ ⟨n, h⟩) prev.2.1) prev.2.2,
        (dt.legStT_fields (v := v) (aT := aT) mV ⟨n, h⟩ prev.1 (tOf ⟨n, h⟩)
          (dt.spineSemT PR.zero PR.one (dt.varAt ⟨n, h⟩) (v := v) mV
            (sem₀ ⟨n, h⟩) prev.2.1) prev.2.2).1.trans prev.2.1,
        dt.legCtlT (v := v) (aT := aT) mV ⟨n, h⟩ prev.1 (tOf ⟨n, h⟩)
          (dt.spineSemT PR.zero PR.one (dt.varAt ⟨n, h⟩) (v := v) mV
            (sem₀ ⟨n, h⟩) prev.2.1) prev.2.2⟩
    else prev
...
noncomputable def spineStOfT (k : Fin (dt.nv + 1)) :
    TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) :=
  (dt.spineNodeT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf (k : ℕ)).1

/-- The threaded control family of the spine. -/
noncomputable def spineFsOfT (k : Fin (dt.nv + 1)) : dt.CtlIx → A :=
  (dt.spineNodeT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf (k : ℕ)).2.2

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
/-- **The mirror rides the threaded family too** — by construction. -/
theorem spineStOfT_mir (k : Fin (dt.nv + 1)) :
    (dt.spineStOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf k).mir = st₀.mir :=
  (dt.spineNodeT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf (k : ℕ)).2.1
...
noncomputable def spineSemOfT (j : Fin dt.nv)
    ...
    (hp : ∀ ℓ : Fin (dt.nIn (dt.varAt j)),
      dt.igPassP PR.zero PR.one (dt.varAt j)
        (dt.varRdSt (dt.spineStOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
          j.castSucc) p (mV a)) ℓ)
    (b : Fin (dt.natOf (dt.varAt j))) :
    dt.KindSem PR.zero PR.one (dt.varAt j)
      (dt.matSt (dt.varAt j)
      ...
      (dt.kindOf (dt.varAt j) b) :=
  dt.spineSemT PR.zero PR.one (dt.varAt j) (v := v) mV (sem₀ j)
    (dt.spineStOfT_mir (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc)
    p a hp b

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem spineStOfT_zero :
    dt.spineStOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf 0 = st₀ := rfl

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem spineFsOfT_zero :
    dt.spineFsOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf 0 = f₀ := rfl
...
theorem spineFsOfT_succ (j : Fin dt.nv) :
    dt.spineFsOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.succ =
      dt.legCtlT (v := v) (aT := aT) mV j
        (dt.spineStOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc)
        (tOf j)
        (dt.spineSemOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j)
        (dt.spineFsOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc) := by
  change (dt.spineNodeT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
    ((j : ℕ) + 1)).2.2 = _
...
theorem spineStOfT_succ (j : Fin dt.nv) :
    dt.spineStOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.succ =
      dt.legStT (v := v) (aT := aT) mV j
        (dt.spineStOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc)
        (tOf j)
        (dt.spineSemOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j)
        (dt.spineFsOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf j.castSucc) := by
...
(`DescriptiveComplexity.Pfp.PfpData.legStB`), and is otherwise the same
...
the *conditioned* family `DescriptiveComplexity.Pfp.PfpData.gatedSem`
...
`DescriptiveComplexity.Pfp.PfpData.stEndB` runs this spine at `v := w` for
...
noncomputable def spineNodeB :
  ...
  | n + 1 =>
    ...
    if h : n < dt.nv then
      ⟨dt.legStB (v := v) (aT := aT) mV ⟨n, h⟩ prev.1
          (semB v ⟨n, h⟩ prev.1) prev.2.2,
        (dt.legStB_fields (v := v) (aT := aT) mV ⟨n, h⟩ prev.1
          (semB v ⟨n, h⟩ prev.1) prev.2.2).1.trans prev.2.1,
        dt.legCtlB (v := v) (aT := aT) mV ⟨n, h⟩ prev.1
          (semB v ⟨n, h⟩ prev.1) prev.2.2⟩
    else prev
...
noncomputable def spineSemOfB (j : Fin dt.nv)
    (hg : dt.gatedAt (PR := PR) j
      (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc))
    (p : Scratch dt A R (OuterPh (EvalPh dt.nv dt.PMF))) (a : ιV)
    (hp : ∀ ℓ : Fin (dt.nIn (dt.varAt j)),
      dt.igPassP PR.zero PR.one (dt.varAt j)
        (dt.varRdSt (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB
          j.castSucc) p (mV a)) ℓ)
    (b : Fin (dt.natOf (dt.varAt j))) :
    dt.KindSem PR.zero PR.one (dt.varAt j)
      (dt.matSt (dt.varAt j)
        (dt.varRdSt (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB
          j.castSucc) p (mV a)) v (b : ℕ))
      (dt.kindOf (dt.varAt j) b) :=
  semB v j (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc) hg p a
    hp b

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem spineStOfB_zero :
    dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB 0 = st₀ := rfl

omit [Finite R] [Finite (OuterPh (EvalPh dt.nv dt.PMF))] [Finite dt.KIx]
  [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem spineFsOfB_zero :
    dt.spineFsOfB (v := v) (aT := aT) mV st₀ f₀ semB 0 = f₀ := rfl
...
theorem spineFsOfB_succ (j : Fin dt.nv) :
    dt.spineFsOfB (v := v) (aT := aT) mV st₀ f₀ semB j.succ =
      dt.legCtlB (v := v) (aT := aT) mV j
        (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc)
        (dt.spineSemOfB (v := v) (aT := aT) mV st₀ f₀ semB j)
        (dt.spineFsOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc) := by
  change (dt.spineNodeB (v := v) (aT := aT) mV st₀ f₀ semB ((j : ℕ) + 1)).2.2 = _
...
theorem spineStOfB_succ (j : Fin dt.nv) :
    dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB j.succ =
      dt.legStB (v := v) (aT := aT) mV j
        (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc)
        (dt.spineSemOfB (v := v) (aT := aT) mV st₀ f₀ semB j)
        (dt.spineFsOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc) := by
  change (dt.spineNodeB (v := v) (aT := aT) mV st₀ f₀ semB ((j : ℕ) + 1)).1 = _
  rw [spineNodeB, dif_pos j.isLt]
  rfl
...
noncomputable def spineBitOfB (j : Fin dt.nv) : Prop :=
  dt.legBitB (v := v) (aT := aT) mV j
    (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc)
    (dt.spineSemOfB (v := v) (aT := aT) mV st₀ f₀ semB j)
    (dt.spineFsOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc)
...
lemmas ask of a leg (`DescriptiveComplexity.Pfp.PfpData.legStB_new`). -/
theorem spineStOfB_writesNew :
    dt.WritesNew (v := v) (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB)
      (dt.spineBitOfB (v := v) (aT := aT) mV st₀ f₀ semB) := by
  intro j i' r
  rw [dt.spineStOfB_succ (v := v) (aT := aT) mV st₀ f₀ semB j,
    dt.legStB_new (v := v) (aT := aT) mV j _ _ _ i' r]
  rfl
      ...
      (semT : dt.gatedAt (PR := PR) j st →
        ...
        ∀ b : Fin (dt.natOf (dt.varAt j)),
          dt.KindSem PR.zero PR.one (dt.varAt j)
            ...
            (dt.kindOf (dt.varAt j) b))
      (f : dt.CtlIx → A),
      F (dt.legStB (v := v) (aT := aT) mV j st semT f) = F st)
    (k : Fin (dt.nv + 1)) :
    F (dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB k) = F st₀ := by
  obtain ⟨n, hn⟩ := k
  induction n with
  | zero => rfl
  | succ n ih =>
    have hnv : n < dt.nv := Nat.lt_of_succ_lt_succ hn
    have hstep := dt.spineStOfB_succ (v := v) (aT := aT) mV st₀ f₀ semB
      ⟨n, hnv⟩
    exact Eq.trans (congrArg F hstep) (Eq.trans (hF _ _ _ _) (ih _))
...
twin of `DescriptiveComplexity.Pfp.PfpData.spineRide` at the legs the
      ...
      (semT : ∀ (p : Scratch dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
        ...
        ∀ b : Fin (dt.natOf (dt.varAt j)),
          dt.KindSem PR.zero PR.one (dt.varAt j)
            ...
            (dt.kindOf (dt.varAt j) b))
      (f : dt.CtlIx → A),
      F (dt.legStT (v := v) (aT := aT) mV j st tOf' semT f) = F st)
    (k : Fin (dt.nv + 1)) :
    F (dt.spineStOfT (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf k) = F st₀ := by
  obtain ⟨n, hn⟩ := k
  induction n with
  | zero => rfl
  | succ n ih =>
    have hnv : n < dt.nv := Nat.lt_of_succ_lt_succ hn
    have hstep := dt.spineStOfT_succ (v := v) (aT := aT) mV st₀ f₀ sem₀ tOf
      ⟨n, hnv⟩
...
theorem new_last_trackOf_B
    ...
    (i : dt.d.B.ι) :
    ((dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀
        (fun w j st hg => dt.gatedSem PR.zero_ne_one hlin mV (v := w) j st hg)
  ...
  set semB : ∀ (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd →
      Prop) (j : Fin dt.nv)
      (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF))),
      dt.gatedAt (PR := PR) j st → _ :=
    fun w j st hg => dt.gatedSem PR.zero_ne_one hlin mV (v := w) j st hg
    with hsemB
  set stj := dt.spineStOfB (v := v) (aT := aT) mV st₀ f₀ semB j.castSucc
    with hstj
  have hmirj : stj.mir = st₀.mir :=
    dt.spineStOfB_mir mV st₀ f₀ semB j.castSucc
  have holdj : stj.old = st₀.old :=
    dt.spineRideB mV st₀ f₀ semB (fun st => st.old)
      (fun j' st' semT f' =>
        (dt.legStB_fields (aT := aT) mV j' st' semT f').2.2.2.1) j.castSucc
  have hwr := dt.spineStOfB_writesNew (v := v) (aT := aT) mV st₀ f₀ semB
  by_cases hg : dt.gatedAt (PR := PR) j stj
  · -- the blocks below this variable's arity encode: the cell is one step
    have hmb : ∀ ℓ : Fin (dt.arOf (dt.varAt j)),
        dt.mirBlk st₀ (Fin.castLE (dt.arOf_le_ko (dt.varAt j)) ℓ) =
          encMap dt.ly PR.zero PR.one
            ((dt.isEnc_of_gatedAt PR.zero_ne_one hlin j stj hg ℓ).choose) :=
      fun ℓ => (congrFun (mirBlk_of_mir hmirj) _).symm.trans
        ((dt.isEnc_of_gatedAt PR.zero_ne_one hlin j stj hg ℓ).choose_spec)
    refine (dt.new_last_next_at mV hlin hord hbotV hmV0 hIncr hKin hTop σ j
      (dt.gatedSem₀ PR.zero_ne_one hlin mV j stj hg)
      (dt.varFG (PR := PR) (dt.varAt j) stj v (dt.tagAt (PR := PR) j stj)
        (dt.spineFsOfB mV st₀ f₀ semB j.castSucc))
      hwr hmirj holdj
      (iff_of_eq (dt.legBitB_gatedSem PR.zero_ne_one hlin hreg mV j stj hg
        (dt.spineFsOfB mV st₀ f₀ semB j.castSucc)))
  ...
  · -- one of them does not: the position writes `False`, and so does the
    -- dictionary
    have hbit : ¬dt.spineBitOfB (v := v) (aT := aT) mV st₀ f₀ semB j := by
        ...
        ¬IsEnc dt.ly PR.zero PR.one
          (wmBlk stj.mir
            (PfpTag.arg (toLex ((Sum.inl
              ...
              PfpTag R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx)) := by
      by_contra hc
      exact hg (dt.gatedAt_of_isEnc PR.zero_ne_one hlin j stj
...
variable (stE : (Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd →
    Prop) → TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) →
  (dt.CtlIx → A) → TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
...
the shape `DescriptiveComplexity.Pfp.PfpData.reaches_sweep` demands. -/
noncomputable def sweepPairG
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop) :
    TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) × (dt.CtlIx → A) :=
...
`DescriptiveComplexity.Pfp.PfpData.sweepStEG_mir` for an evaluation that
...
variable (mV : ιV → Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd →
  Prop)
variable (semOf : ∀ (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
  (j : Fin dt.nv) (a : ιV),
  (∀ ℓ : Fin (dt.nIn (dt.varAt j)),
    dt.igPassP PR.zero PR.one (dt.varAt j) (dt.roundSt st (mV a)) ℓ) →
  ∀ b : Fin (dt.natOf (dt.varAt j)),
    dt.KindSem PR.zero PR.one (dt.varAt j) (dt.roundSt st (mV a))
      (dt.kindOf (dt.varAt j) b))
variable (tOf : ∀ (_st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
...
noncomputable def stEnd
  ...
  dt.spineStOf (v := w) (aT := aT) mV st f (semOf st) (tOf st)
    (Fin.last dt.nv)
...
noncomputable def fsEnd
  ...
  dt.spineFsOf (v := w) (aT := aT) mV st f (semOf st) (tOf st)
    (Fin.last dt.nv)
...
variable (semAt : ∀ (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx
    dt.dd → Prop)
  (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF))) (j : Fin dt.nv)
  (a : ιV),
  (∀ ℓ : Fin (dt.nIn (dt.varAt j)),
    dt.igPassP PR.zero PR.one (dt.varAt j)
...
variable (tAt : ∀ (_w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx
    dt.dd → Prop)
  (_st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF))) (j : Fin dt.nv),
...
noncomputable def stEndT
    ...
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) :=
  dt.spineStOfT (v := w) (aT := aT) mV { st with mir := w } f (semAt w st)
...
noncomputable def fsEndT
    ...
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) : dt.CtlIx → A :=
  dt.spineFsOfT (v := w) (aT := aT) mV { st with mir := w } f (semAt w st)
    (tAt w st) (Fin.last dt.nv)
...
theorem stEndT_mir
    ...
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) :
    (dt.stEndT (aT := aT) mV semAt tAt w st f).mir = w :=
  dt.spineStOfT_mir (v := w) (aT := aT) mV { st with mir := w } f (semAt w st)
    (tAt w st) (Fin.last dt.nv)
...
theorem stEndT_ride {β : Sort _}
    (F : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) → β)
    (hFmir : ∀ (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF))) m,
      ...
      (semT : ∀ (p : Scratch dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
        ...
        ∀ b : Fin (dt.natOf (dt.varAt j)),
          dt.KindSem PR.zero PR.one (dt.varAt j)
            ...
            (dt.kindOf (dt.varAt j) b))
      (f : dt.CtlIx → A),
      F (dt.legStT (v := w) (aT := aT) mV j st tOf' semT f) = F st)
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) :
    F (dt.stEndT (aT := aT) mV semAt tAt w st f) = F st :=
  (dt.spineRideT (v := w) (aT := aT) mV { st with mir := w } f (semAt w st)
...
of `DescriptiveComplexity.Pfp.PfpData.gatedSem`, quantified over the address
as well: `DescriptiveComplexity.Pfp.PfpData.stEndB` runs the spine at
...
noncomputable def stEndB
    ...
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) :=
  dt.spineStOfB (v := w) (aT := aT) mV { st with mir := w } f semAtB
    (Fin.last dt.nv)
...
noncomputable def fsEndB
    ...
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) : dt.CtlIx → A :=
  dt.spineFsOfB (v := w) (aT := aT) mV { st with mir := w } f semAtB
    (Fin.last dt.nv)
...
theorem stEndB_mir
    ...
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) :
    (dt.stEndB (aT := aT) mV semAtB w st f).mir = w :=
  dt.spineStOfB_mir (v := w) (aT := aT) mV { st with mir := w } f semAtB
    (Fin.last dt.nv)
      ...
      (semT : dt.gatedAt (PR := PR) j st →
        ...
        ∀ b : Fin (dt.natOf (dt.varAt j)),
          dt.KindSem PR.zero PR.one (dt.varAt j)
            ...
            (dt.kindOf (dt.varAt j) b))
      (f : dt.CtlIx → A),
      F (dt.legStB (v := w) (aT := aT) mV j st semT f) = F st)
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (st : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)))
    (f : dt.CtlIx → A) :
    F (dt.stEndB (aT := aT) mV semAtB w st f) = F st :=
  (dt.spineRideB (v := w) (aT := aT) mV { st with mir := w } f semAtB
...
theorem stEndB_new_off
    ...
    {r : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop}
    (hr : r ≠ w) :
    (dt.stEndB (aT := aT) mV semAtB w st f).new i r ↔ st.new i r :=
  dt.spine_new_off (dt.spineStOfB_writesNew (v := w) (aT := aT) mV
...
shape `DescriptiveComplexity.Pfp.PfpData.reaches_sweep` demands. -/
noncomputable def sweepPair
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop) :
    TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) × (dt.CtlIx → A) :=
  addrIter (isLinOrd_wmSetLe hlin) (st₀, f₀)
    (fun u p =>
      ({ dt.atSt (dt.stEnd (aT := aT) mV semOf tOf u p.1 p.2)
            (wmNext hlin u) with mir := wmNext hlin u },
        dt.fsEnd (aT := aT) mV semOf tOf u p.1 p.2)) w

/-- The sweep's tape family — `reaches_sweep`'s `SW`. -/
noncomputable def sweepSW
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop) :
    TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) :=
  (dt.sweepPair (aT := aT) mV semOf tOf hlin st₀ f₀ w).1

/-- The sweep's control family — `reaches_sweep`'s `FS`. -/
noncomputable def sweepFS
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop) :
    dt.CtlIx → A :=
  (dt.sweepPair (aT := aT) mV semOf tOf hlin st₀ f₀ w).2
...
noncomputable def sweepStE
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop) :
    TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)) :=
  dt.stEnd (aT := aT) mV semOf tOf w
    (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w)
    (dt.sweepFS (aT := aT) mV semOf tOf hlin st₀ f₀ w)
...
noncomputable def sweepFsE
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop) :
    dt.CtlIx → A :=
  dt.fsEnd (aT := aT) mV semOf tOf w
    (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w)
    (dt.sweepFS (aT := aT) mV semOf tOf hlin st₀ f₀ w)

omit [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem sweepSW_bot :
    dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ (fun _ => False) = st₀ :=
  congrArg Prod.fst
    (addrIter_bot hlin (isLinOrd_wmSetLe hlin) (st₀, f₀) _)

omit [LinearOrder (dt.X.Map A)] [Finite ιV] in
theorem sweepFS_bot :
    dt.sweepFS (aT := aT) mV semOf tOf hlin st₀ f₀ (fun _ => False) = f₀ :=
  congrArg Prod.snd
    (addrIter_bot hlin (isLinOrd_wmSetLe hlin) (st₀, f₀) _)
...
theorem sweepSW_incr
    ...
    (hi : WMIncr WMLe w w') :
    dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w' =
      { dt.atSt (dt.sweepStE (aT := aT) mV semOf tOf hlin st₀ f₀ w) w' with
        mir := w' } := by
  have h := congrArg Prod.fst
    (addrIter_incr hlin (isLinOrd_wmSetLe hlin) hi (st₀, f₀)
      (fun u p =>
        ({ dt.atSt (dt.stEnd (aT := aT) mV semOf tOf u p.1 p.2)
              (wmNext hlin u) with mir := wmNext hlin u },
          dt.fsEnd (aT := aT) mV semOf tOf u p.1 p.2)))
  rw [wmNext_eq hlin hi] at h
  exact h
...
theorem sweepFS_incr
    ...
    (hi : WMIncr WMLe w w') :
    dt.sweepFS (aT := aT) mV semOf tOf hlin st₀ f₀ w' =
      dt.sweepFsE (aT := aT) mV semOf tOf hlin st₀ f₀ w :=
  congrArg Prod.snd
    (addrIter_incr hlin (isLinOrd_wmSetLe hlin) hi (st₀, f₀)
      (fun u p =>
        ({ dt.atSt (dt.stEnd (aT := aT) mV semOf tOf u p.1 p.2)
...
theorem sweepStE_ride {β : Sort _}
    ...
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop) :
    F (dt.sweepStE (aT := aT) mV semOf tOf hlin st₀ f₀ w) =
      F (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w) :=
  dt.spineRide
    (hst := fun j => dt.spineStOf_succ (v := w) (aT := aT) mV
      (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w)
      (dt.sweepFS (aT := aT) mV semOf tOf hlin st₀ f₀ w)
      (semOf (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w))
      (tOf (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w)) j)
...
theorem sweepSW_ride {β : Sort _}
    ...
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁) :
    F (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w) = F st₀ := by
  refine holds_of_wideRounds hlin
    (Q := fun u => F (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ u) =
      F st₀) ?_ ?_ w hlb hub
  · rw [dt.sweepSW_bot (aT := aT) mV semOf tOf hlin st₀ f₀]
  · intro u u' hi _ _ ih
    rw [dt.sweepSW_incr (aT := aT) mV semOf tOf hlin st₀ f₀ hi, hFa,
      dt.sweepStE_ride (aT := aT) mV semOf tOf hlin st₀ f₀ F hF u]
    exact ih
...
theorem sweepSW_wk (hwk₀ : st₀.wk = fun r => r = (fun _ => False))
    ...
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁) :
    (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w).wk = fun r => r = w := by
  refine holds_of_wideRounds hlin
    (Q := fun u => (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ u).wk =
      fun r => r = u) ?_ ?_ w hlb hub
  · rw [dt.sweepSW_bot (aT := aT) mV semOf tOf hlin st₀ f₀]
    exact hwk₀
  · intro u u' hi _ _ _
    rw [dt.sweepSW_incr (aT := aT) mV semOf tOf hlin st₀ f₀ hi]
    rfl
...
theorem sweepSW_mir (hmir₀ : st₀.mir = fun _ => False)
    ...
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁) :
    (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w).mir = w := by
  refine holds_of_wideRounds hlin
    (Q := fun u => (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ u).mir = u)
    ?_ ?_ w hlb hub
  · rw [dt.sweepSW_bot (aT := aT) mV semOf tOf hlin st₀ f₀]
    exact hmir₀
  · intro u u' hi _ _ _
...
omit [Finite ιV] [LinearOrder (dt.X.Map A)] in
/-- **`DescriptiveComplexity.Pfp.PfpData.reaches_sweep`'s `hwkE`**: the
marker is still at the address when the address's spine ends. -/
theorem sweepStE_wk (hwk₀ : st₀.wk = fun r => r = (fun _ => False))
    {s₁ : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop}
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁) :
    (dt.sweepStE (aT := aT) mV semOf tOf hlin st₀ f₀ w).wk = fun r => r = w :=
  (dt.sweepStE_ride (aT := aT) mV semOf tOf hlin st₀ f₀
    (fun st => st.wk) (fun _ _ _ _ _ => rfl) w).trans
    (dt.sweepSW_wk (aT := aT) mV semOf tOf hlin st₀ f₀ hwk₀ (s₁ := s₁) w hlb hub)
...
theorem sweepStE_mir (hmir₀ : st₀.mir = fun _ => False)
    ...
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁) :
    (dt.sweepStE (aT := aT) mV semOf tOf hlin st₀ f₀ w).mir = w :=
  (dt.sweepStE_ride (aT := aT) mV semOf tOf hlin st₀ f₀
    (fun st => st.mir) (fun _ _ _ _ _ => rfl) w).trans
    (dt.sweepSW_mir (aT := aT) mV semOf tOf hlin st₀ f₀ hmir₀ (s₁ := s₁) w hlb hub)
...
theorem sweepStE_ltp_eq
    ...
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁) :
    (dt.sweepStE (aT := aT) mV semOf tOf hlin st₀ f₀ w).ltp = st₀.ltp :=
  (dt.sweepStE_ride (aT := aT) mV semOf tOf hlin st₀ f₀
    (fun st => st.ltp) (fun _ _ _ _ _ => rfl) w).trans
    (dt.sweepSW_ride (aT := aT) mV semOf tOf hlin st₀ f₀ (fun st => st.ltp)
...
theorem sweepStE_ltp
    ...
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁)
    (hltp₀ : ¬st₀.ltp w) :
    ¬(dt.sweepStE (aT := aT) mV semOf tOf hlin st₀ f₀ w).ltp w := by
  rw [dt.sweepStE_ltp_eq (aT := aT) mV semOf tOf hlin st₀ f₀ (s₁ := s₁) w
    hlb hub]
  exact hltp₀

/-! ### The SAV/TGT gap

`DescriptiveComplexity.Pfp.PfpData.evalSpine_run` asks, at every address, for
`hsavOf`/`htgtOf` — the SAV and TARGET registers holding *that address* —
because `DescriptiveComplexity.Pfp.PfpData.matrix_run` reads them there. But
...
theorem sweepSW_sav
    ...
    (w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop)
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁) :
    (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w).sav = st₀.sav :=
  dt.sweepSW_ride (aT := aT) mV semOf tOf hlin st₀ f₀ (fun st => st.sav)
    (fun _ _ _ _ _ => rfl) (fun _ _ => rfl) (s₁ := s₁) w hlb hub
...
theorem eq_of_sweepSW_sav
    ...
    (hlb₁ : WMSetLe WMLe (fun _ => False) w₁) (hub₁ : WMSetLe WMLe w₁ s₁)
    (hlb₂ : WMSetLe WMLe (fun _ => False) w₂) (hub₂ : WMSetLe WMLe w₂ s₁)
    (h₁ : (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w₁).sav = w₁)
    (h₂ : (dt.sweepSW (aT := aT) mV semOf tOf hlin st₀ f₀ w₂).sav = w₂) :
    w₁ = w₂ := by
  rw [← h₁, ← h₂,
    dt.sweepSW_sav (aT := aT) mV semOf tOf hlin st₀ f₀ (s₁ := s₁) w₁ hlb₁ hub₁,
    dt.sweepSW_sav (aT := aT) mV semOf tOf hlin st₀ f₀ (s₁ := s₁) w₂ hlb₂ hub₂]
...
theorem sweep_new_trackOf
      ...
      (∀ (i : dt.d.B.ι)
        ...
        WMSetLe WMLe (fun _ => False) r → WMSetLt WMLe r w →
          ((dt.sweepSWG
              (dt.stEndB (aT := aT) mV
                (fun w' j st hg =>
                  dt.gatedSem PR.zero_ne_one hlin mV (v := w') j st hg))
              (dt.fsEndB (aT := aT) mV
                (fun w' j st hg =>
                  dt.gatedSem PR.zero_ne_one hlin mV (v := w') j st hg))
              hlin st₀ f₀ w).new i r ↔
  ...
  have hold : ∀ w, WMSetLe WMLe (fun _ => False) w → WMSetLe WMLe w s₁ →
      (dt.sweepSWG (dt.stEndB (aT := aT) mV semG)
        (dt.fsEndB (aT := aT) mV semG) hlin st₀ f₀ w).old = st₀.old :=
    fun w hlb hub => dt.sweepSWG_ride _ _ hlin st₀ f₀ (fun st => st.old)
      (fun u st f => dt.stEndB_ride (aT := aT) mV semG (fun st => st.old)
        (fun _ _ => rfl)
        (fun _ _ _ _ _ =>
          (dt.legStB_fields (aT := aT) mV _ _ _ _).2.2.2.1) u st f)
  ...
  have hreg : ∀ w, WMSetLe WMLe w s₁ →
      ¬∃ u : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd,
        w = wmSeg u := by
    intro w hub
    refine not_wmSeg_of_wmSetLt_wmSeg hlin hbot ((wmSetLt_iff _ _).mpr
      ⟨hset.2.1 w s₁ (wmSeg gbot) hub ((wmSetLt_iff _ _).mp hs₁).1, fun hc => ?_⟩)
    exact ((wmSetLt_iff _ _).mp hs₁).2
      (hset.2.2.1 s₁ (wmSeg gbot) ((wmSetLt_iff _ _).mp hs₁).1 (hc ▸ hub))
  have key := dt.sweep_new hlin
    (s₀ := fun _ => False) (s₁ := s₁)
    (SW := dt.sweepSWG (dt.stEndB (aT := aT) mV semG)
      (dt.fsEndB (aT := aT) mV semG) hlin st₀ f₀)
    (stE := dt.sweepStEG (dt.stEndB (aT := aT) mV semG)
      (dt.fsEndB (aT := aT) mV semG) hlin st₀ f₀)
    (N := fun i r =>
  ...
  · intro w hlb hub
    ...
    rw [dt.sweepSWG_bot (dt.stEndB (aT := aT) mV semG)
      (dt.fsEndB (aT := aT) mV semG) hlin st₀ f₀]
  · -- the address's own cell, by the branched spine's dictionary
    have hub' : WMSetLe WMLe w s₁ := ((wmSetLt_iff _ _).mp hub).1
    exact dt.new_last_trackOf_B mV
      { dt.sweepSWG (dt.stEndB (aT := aT) mV semG)
          (dt.fsEndB (aT := aT) mV semG) hlin st₀ f₀ w with mir := w }
      (dt.sweepFSG (dt.stEndB (aT := aT) mV semG)
        (dt.fsEndB (aT := aT) mV semG) hlin st₀ f₀ w)
  ...
  · -- every other cell rides the address's own evaluation
    exact dt.stEndB_new_off (aT := aT) mV semG w _ _ i hr
...
include hlin in
/-- **`DescriptiveComplexity.Pfp.PfpData.reaches_sweep`'s `hspine`, at one
...
theorem reaches_spineB
    ...
    (hIncr : ∀ a a' : ιV, a < a' → (∀ b, ¬(a < b ∧ b < a')) →
      WMIncr WMLe (mV a) (mV a'))
    (hTestT : ∀ u, dt.InnerFull (mV aT) u)
    (hTestF : ∀ a, a < aT → ∃ u, ¬dt.InnerFull (mV a) u)
    (hwk₀ : st₀.wk = fun r => r = (fun _ => False))
    (hmir₀ : st₀.mir = fun _ => False)
    (hbot₀ : st₀.bot = fun r => r = (fun _ => False))
    {s₁ w : Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd → Prop}
    (hlb : WMSetLe WMLe (fun _ => False) w) (hub : WMSetLe WMLe w s₁)
    (hv : WMSetLt WMLe w (wmSeg gbot)) (hvi : WMIncr WMLe w v') :
    Relation.ReflTransGen
      (wideData (Univ A R (OuterPh (EvalPh dt.nv dt.PMF)) dt.KIx dt.dd)).Step
      ⟨Sum.inr (PR.stElt (.evalP (.chk 0))
          (dt.sweepFSG (dt.stEndB (aT := aT) mV semAtB)
            (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w)), Sum.inl w,
        wideTape (PR.trackTape Slot.val
          (dt.back PR.zero PR.one dt.dd0Le
            (dt.sweepSWG (dt.stEndB (aT := aT) mV semAtB)
              (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w))
          (dt.sweepSWG (dt.stEndB (aT := aT) mV semAtB)
            (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w).val)
          (PR.syElt PR.blank)⟩
      ⟨Sum.inr (PR.stElt (.evalP (.chk (Fin.last dt.nv)))
          (dt.sweepFsEG (dt.stEndB (aT := aT) mV semAtB)
            (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w)), Sum.inl w,
        wideTape (PR.trackTape Slot.val
          (dt.back PR.zero PR.one dt.dd0Le
            (dt.sweepStEG (dt.stEndB (aT := aT) mV semAtB)
              (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w))
          (dt.sweepStEG (dt.stEndB (aT := aT) mV semAtB)
            (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w).val)
          (PR.syElt PR.blank)⟩ := by
  classical
  have hmirSW : (dt.sweepSWG (dt.stEndB (aT := aT) mV semAtB)
      (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w).mir = w :=
    dt.sweepSWG_mir _ _ hlin st₀ f₀ hmir₀ (s₁ := s₁) w hlb hub
  have hwkSW : (dt.sweepSWG (dt.stEndB (aT := aT) mV semAtB)
      (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w).wk = fun r => r = w :=
    dt.sweepSWG_wk _ _ hlin st₀ f₀ hwk₀ (s₁ := s₁) w hlb hub
  have hbotSW : (dt.sweepSWG (dt.stEndB (aT := aT) mV semAtB)
      (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w).bot =
      fun r => r = (fun _ => False) := by
    refine Eq.trans (dt.sweepSWG_ride _ _ hlin st₀ f₀ (fun st => st.bot) ?_
      (fun _ _ => rfl) (s₁ := s₁) w hlb hub) hbot₀
    exact fun u st f => dt.stEndB_ride (aT := aT) mV semAtB (fun st => st.bot)
      (fun _ _ => rfl)
      (fun _ _ _ _ _ => (dt.legStB_fields (aT := aT) mV _ _ _ _).2.2.1) u st f
  -- pinning the mirror at the address is the identity on the entry state
  have hpin : ∀ X : TapeSt dt A R (OuterPh (EvalPh dt.nv dt.PMF)),
      X.mir = w → ({ X with mir := w } : TapeSt dt A R
        (OuterPh (EvalPh dt.nv dt.PMF))) = X := by
    intro X hX
    rw [← hX]
  rw [← hpin _ hmirSW]
  refine dt.evalSpineB_run hrules hR hlin hord htop hbot hv hvi hbotV htopV
    mV hmV0 hIncr hTestT hTestF
    (stOf := dt.spineStOfB (v := w) (aT := aT) mV
      { dt.sweepSWG (dt.stEndB (aT := aT) mV semAtB)
          (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w with mir := w }
      (dt.sweepFSG (dt.stEndB (aT := aT) mV semAtB)
        (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w)
      semAtB)
    (fsOf := dt.spineFsOfB (v := w) (aT := aT) mV
      { dt.sweepSWG (dt.stEndB (aT := aT) mV semAtB)
          (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w with mir := w }
      (dt.sweepFSG (dt.stEndB (aT := aT) mV semAtB)
        (dt.fsEndB (aT := aT) mV semAtB) hlin st₀ f₀ w)
      semAtB)
    ?_ ?_ ?_
    (semTJ := dt.spineSemOfB (v := w) (aT := aT) mV _ _ _)
    (fun j => dt.spineStOfB_succ (v := w) (aT := aT) mV _ _ _ j)
    (fun j => dt.spineFsOfB_succ (v := w) (aT := aT) mV _ _ _ j)
  · intro k
    refine Eq.trans (dt.spineRideB (v := w) (aT := aT) mV _ _ _
      (fun st => st.wk)
      (fun _ _ _ _ => (dt.legStB_fields (aT := aT) mV _ _ _ _).2.1) k) hwkSW
  · intro j
    exact dt.spineStOfB_mir (v := w) (aT := aT) mV _ _ _ j.castSucc
  · intro j
    refine Eq.trans (dt.spineRideB (v := w) (aT := aT) mV _ _ _
      (fun st => st.bot)
      (fun _ _ _ _ => (dt.legStB_fields (aT := aT) mV _ _ _ _).2.2.1)
      j.castSucc) hbotSW
...
`DescriptiveComplexity.Pfp.PfpData.stageEndSt st v = { st with sav := v,
