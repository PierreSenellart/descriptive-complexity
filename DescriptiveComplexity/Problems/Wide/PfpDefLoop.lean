...
`DescriptiveComplexity.Pfp.tupNext`, the lexicographic successor: the
...
* Which coordinate rolls over is decided by *which coordinates are maximal*,
  ...
  pattern (`DescriptiveComplexity.Pfp.uSlotDefinable_cases`).
...
`DescriptiveComplexity.Pfp.tupNext_apply_of_carry` is the coordinatewise
