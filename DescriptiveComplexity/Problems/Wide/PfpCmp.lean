...
* `DescriptiveComplexity.Pfp.PfpData.encMap_eq_iff_padBits` – two encodings
  are equal exactly when they agree at every padded tuple, because an
  encoding is supported on padded tuples
  (`DescriptiveComplexity.Pfp.PfpData.isPad_of_encMap`) and the encoding is
  injective. So the equality atom is *the loop found no difference*.
* `DescriptiveComplexity.Pfp.PfpData.wmSetLe_encMap_iff_padBits` – the
...
* `DescriptiveComplexity.Pfp.PfpData.encOrder_le_iff_padBits` reads the same
  through the order the reduction puts on the points
  (`DescriptiveComplexity.Pfp.encOrder`), which is where an order atom of the
...
(`DescriptiveComplexity.Pfp.encOrder`). -/
