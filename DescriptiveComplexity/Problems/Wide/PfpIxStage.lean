...
(`DescriptiveComplexity.Pfp.PfpData.stageXS`/`stageXD`). At a coarser file they
are **registers**, named by a block and a tuple
(`DescriptiveComplexity.Pfp.Layout.reg`), what the copy builds is a **mark**,
...
leg of `DescriptiveComplexity.Pfp.PfpData.stage_reachesIn`, at one loop and one
...
theorem ixStageTgt_arg (st : TapeSt dt A R P I) (n : ℕ) {u : I}
    ...
    ∃ i : dt.KIx, (elt u).1 = PfpTag.arg i := by
  classical
  ...
  | succ n ih =>
    ...
    · simp only [tuplePost] at hy
      ...
      · exact tupleIterD_of_mem (fun u => ∃ i : dt.KIx, (elt u).1 = PfpTag.arg i)
...
it is `DescriptiveComplexity.Pfp.PfpData.stage_run`, which takes the file as a
...
theorem wmBlk_ixStageTgt_eq_encMap {one : A} (hinj : Function.Injective elt)
    ...
    wmBlk (ixAddr elt (dt.ixStageTgt F hhas vi ts st (dt.d.B.arity iv)))
        (PfpTag.arg (toLex ((Sum.inl (Fin.castLE (dt.arOf_le_ko (some iv)) ℓ) :
  ...
  · have hpadded : w = pad (dd := dt.dd) zero (unpad dt.dd0Le w) :=
    ...
    have hsrcbit : wmBlk (ixAddr elt (dt.lvSet st vi (ts ℓ)))
        (PfpTag.arg (toLex (dt.lvBlk vi (ts ℓ))) : PfpTag R P dt.KIx) w := by
      rw [hsrc ℓ]; exact h
