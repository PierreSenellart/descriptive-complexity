...
(`DescriptiveComplexity.Pfp.PfpData.nexEval_reachesIn`) – and this file puts
them together and compares the sum with the clock.

The opening is `DescriptiveComplexity.Pfp.PfpData.reachesIn_openingRegion`,
...
(`DescriptiveComplexity.Pfp.trackTape_of_back`), so the caller hands that
...
`DescriptiveComplexity.Pfp.Prog.trackTapeAt_initBack` says the all-blank tape is
the pass-layer presentation at *any* file – which a clocked program needs,
because its file does not exist at time zero – and
`DescriptiveComplexity.Pfp.PfpData.startBack` with `startBack_frame` /
...
`DescriptiveComplexity.Pfp.PfpData.nexIxEvalB_reachesIn`. -/
...
`DescriptiveComplexity.Pfp.PfpData.nexProg_wideAccept_of_legs` does the clock. -/
...
(`DescriptiveComplexity.Pfp.Prog.wideAccept_prog` presents it along the
...
variable {L : Language.{0, 0}} {dt : PfpData L} {A R' PE SE : Type}
...
on a blank tape, so this – with `DescriptiveComplexity.Pfp.Prog.initBack` for the
...
theorem wmSetLe_succ_bot_logicalTop [Nonempty A]
  ...
  wmSetLe_succ_bot_of_nonempty h hs₀
    ⟨(PfpTag.arg i₀, fun _ => Classical.arbitrary A), i₀, rfl⟩
...
(`DescriptiveComplexity.Pfp.PfpData.exists_postGuess_shaped`). From there on the
...
`DescriptiveComplexity.Pfp.PfpData.nexTotal_lt_two_pow'` asks of it. The region
...
(`DescriptiveComplexity.Pfp.wideRank_lt_two_pow_logical`). What is left to check
...
theorem openingLogical_le_two_pow {A R P K : Type} {dd : ℕ}
    ...
    (htop : ∀ τ : PfpTag R P K, (∀ i : K, τ ≠ PfpTag.arg i) →
      ∀ v : Fin dd → A, ¬top (τ, v))
...
of addresses (`DescriptiveComplexity.Pfp.PfpData.nexTotal_lt_two_pow'`). -/
