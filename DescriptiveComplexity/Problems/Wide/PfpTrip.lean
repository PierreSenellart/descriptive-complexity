...
file test, a track write. `DescriptiveComplexity.Pfp.Prog.reaches_roundTrip`
...
theorem reaches_roundTrip (hR : PR.table.Reads)
    ...
    (hrl : ∀ r : Univ A R P K dd → Prop, rest r rl = bitVal PR.zero PR.one (r = wmSeg gtop))
    ...
    (hb₁ : PR.HasLeft p₁ fc (PR.passTracks t rest m (wmSeg gtop)) p₂b fc
      (PR.passTracks t rest m (wmSeg gtop)))
    (hb₂ : ∀ g : W → A, PR.HasRight p₂b fc g pIn fc g)
    {pend : Univ A R P K dd → Prop} (hpend : WMIncr WMLe pend (wmSeg gbot))
    (hmid : Relation.ReflTransGen (wideData (Univ A R P K dd)).Step
      ⟨Sum.inr (PR.stElt pIn fc), Sum.inl (wmSeg gtop),
        wideTape (PR.trackTape t rest m) (PR.syElt PR.blank)⟩
      ⟨Sum.inr (PR.stElt pOut fc), Sum.inl pend,
        wideTape (PR.trackTape t rest m₂) (PR.syElt PR.blank)⟩)
    (hback : ∀ r : Univ A R P K dd → Prop, (∀ x : Univ A R P K dd, r ≠ wmSeg x) →
      PR.passTracks t rest m₂ r wk ≠ PR.one →
      PR.HasLeft pOut fc (PR.passTracks t rest m₂ r) pOut fc (PR.passTracks t rest m₂ r))
    (hwkLt : WMSetLt WMLe wkAddr (wmSeg gbot))
  ...
  have hscan := reaches_toCell (StopG := fun g => g rl = PR.one) hR hlin
    ...
    (fun g hg => hscanUp g hg) (u := wmSeg gtop) (s := s)
    (by rw [passTracks_of_ne hnerl, hrl]; exact bitVal_pos rfl)
    (wmSetLe_of_full hlin hfull s)
    (fun r _ hstop => by
  ...
  have s₄ := PR.step_move (t := t) (rest := rest) (rest' := rest) (m := m) hR hlin hpt
    ...
    (hb₂ (PR.passTracks t rest m pt))
  -- the return scan: every cell at or below `pend` is nobody's register
  have hnonreg : ∀ r : Univ A R P K dd → Prop, WMSetLe WMLe r pend →
      ∀ x : Univ A R P K dd, r ≠ wmSeg x := by
    intro r hle x hrx
    subst hrx
    exact ((wmSetLt_wmSeg_iff hlin x gbot).mp
      ((wmSetLt_iff_of_wmIncr hlin hpend (wmSeg x)).mpr hle)).2 (hbot x)
  have hle_back : WMSetLe WMLe wkAddr pend :=
    (wmSetLt_iff_of_wmIncr hlin hpend wkAddr).mp hwkLt
  have hret := reaches_toCellBackC (StopG := fun g => g wk = PR.one) hR hlin
