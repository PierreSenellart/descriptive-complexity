...
pack `DescriptiveComplexity.Pfp.PfpData.cmpArgs`. The enumeration is the
lexicographic order on the narrow tuples, the generated family of
`DescriptiveComplexity.Pfp.elemFam` carries the loop element and the three
...
* **the loop element is the round's tuple**
  (`DescriptiveComplexity.Pfp.PfpData.readLv_cmpFam`), so the name guards
...
* **the flags fold the strict prefix**
  (`DescriptiveComplexity.Pfp.PfpData.cmpFlags_cmpIter`): agreement so far,
  a difference seen, and the first difference's verdict, over the tuples
  strictly below the round's.

`DescriptiveComplexity.Pfp.PfpData.cmp_run` is the machine run; the final
control's verdict bit is characterized against the two tracks' padded bits
(`DescriptiveComplexity.Pfp.PfpData.ctlBit_avC_cmp_run`), which is the form
`DescriptiveComplexity.Problems.Wide.PfpCmp` turns into the equality and
order atoms.
-/
...
`DescriptiveComplexity.Pfp.encMap_eq_iff_padBits` and
`DescriptiveComplexity.Pfp.encOrder_le_iff_padBits` decide against the two
registers' contents. -/
