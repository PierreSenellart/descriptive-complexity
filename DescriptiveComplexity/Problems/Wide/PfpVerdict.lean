...
The capstone `DescriptiveComplexity.Pfp.PfpData.accVerdict_varFM_qfValue`
reads the machinery's exit bit as the alternating prefix over an abstract
leaf `Ps`; `DescriptiveComplexity.Pfp.PfpData.altQuantFrom_leafP` reads that
...
is the prefix over `DescriptiveComplexity.Pfp.PfpData.leafP` at the
...
(`hsem`, pinning it to `DescriptiveComplexity.Pfp.PfpData.passSem`) rather
than a fixed choice, because the spine's per-position family
(`DescriptiveComplexity.Pfp.PfpData.evalSpine_run`'s `semOfJ`) is chosen by
...
input of `DescriptiveComplexity.Pfp.PfpData.accVerdict_varFM_qfValue`
discharged — the flags by
`DescriptiveComplexity.Pfp.PfpData.ctlBit_roundFX_pass_iff`, the pass by
`DescriptiveComplexity.Pfp.PfpData.roundPass_of_polarities`, the valuation
and its pack by `DescriptiveComplexity.Pfp.PfpData.passW`, the stage reads
by `DescriptiveComplexity.Pfp.PfpData.old_trackOf_stageTgtD`, and the two
leaf readings by `DescriptiveComplexity.Pfp.PfpData.leafP_pass_iff` /
`DescriptiveComplexity.Pfp.PfpData.leafP_fail_iff`. -/
...
prefix of `DescriptiveComplexity.Pfp.PfpData.accVerdict_leafP` read through
`DescriptiveComplexity.Pfp.PfpData.altQuantFrom_leafP`. -/
theorem accVerdict_next
  ...
  refine (dt.accVerdict_leafP (some i) st mV semOf hlin hord hbotV hmV0
    hIncr hKin hTop σ mbW hmb hdict hordP hsem fG).trans ?_
