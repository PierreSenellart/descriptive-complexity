...
theorem stageSt_old
    ...
    (hKin : ∀ (a : ιV)
      (t : PfpTag (dt.RIx zero one hzo (fun w => dt.varArgsOf zero one w))
        dt.PF dt.KIx)
...
`DescriptiveComplexity.Pfp.PfpData.reaches_startup` stops one rule short of
...
`DescriptiveComplexity.Pfp.PfpData.startupSt` — the bottom mark at the empty
