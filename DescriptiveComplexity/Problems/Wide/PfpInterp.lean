...
space-bounded program's own are `PfpData.SF` and `PfpData.SFSh`, the clocked
...
abbrev ITagOf (dt : PfpData L) (S : Type) (Sh : S → Type) (P : Type) : Type :=
  PfpTag (RTagOf S Sh) P dt.KIx

/-- **The rule names of the space-bounded program.** -/
abbrev RTag (dt : PfpData L) : Type := RTagOf dt.SF dt.SFSh

/-- **The tags of the space-bounded program's interpreted universe.** -/
abbrev ITag (dt : PfpData L) : Type := dt.ITagOf dt.SF dt.SFSh dt.PF
...
The mark of `DescriptiveComplexity.Pfp.regSlotMark` differs from the one above
...
the reading `DescriptiveComplexity.Pfp.isGreatest_tagTupleLe_iff` gives for the
...
theorem isTopNonArg_iff (x : Univ e.α (RTagOf S Sh) P dt.KIx dt.dd) :
    IsTopNonArg x ↔
      ((∀ i, x.1 ≠ PfpTag.arg i) ∧
        (∀ t : PfpTag (RTagOf S Sh) P dt.KIx, (∀ i, t ≠ PfpTag.arg i) → t ≤ x.1) ∧
        ∀ j, IsTop (x.2 j)) := by
  constructor
  · rintro ⟨hna, hall⟩
...
noncomputable def markSlotRegF (t : (dt.ITagOf S Sh P)) (y : Fin 2 × Fin dt.dd) :
    dt.SlotIx → (L.sum Language.order).Formula (Fin 2 × Fin dt.dd)
  | .regFirst =>
    bitAtF y (sideF _ ((∀ i, t ≠ PfpTag.arg i) ∧
        ∀ t' : (dt.ITagOf S Sh P), (∀ i, t' ≠ PfpTag.arg i) → t' ≤ t) ⊓
...
theorem realize_markSlotRegF (t : (dt.ITagOf S Sh P)) (y : Fin 2 × Fin dt.dd)
  ...
  match s with
  | .regFirst =>
    have hP : ((sideF (L := L) _ ((∀ i, t ≠ PfpTag.arg i) ∧
          ∀ t' : (dt.ITagOf S Sh P), (∀ i, t' ≠ PfpTag.arg i) → t' ≤ t) ⊓
        topTupF (argVar 2 dt.dd 0)).Realize v) ↔
...
structure UMarkDef {L : Language.{0, 0}} (dt : PfpData L) [Fintype dt.SlotIx]
...
/-- **The register file**, as a mark: `DescriptiveComplexity.Pfp.slotMark`
defined by `inpF`. -/
...
writes. The content is `DescriptiveComplexity.Pfp.regSlotMark`'s, which differs
...
noncomputable def regFileMarkArg : UMarkDef dt S Sh P where
  ...
  marked _ x := (∃ k : dt.KIx, x.1 = PfpTag.arg k) ∨ IsTopNonArg x
  form t t' :=
    (sideF _ (∃ k : dt.KIx, t = PfpTag.arg k) ⊔
      (sideF _ ((∀ i, t ≠ PfpTag.arg i) ∧
          ∀ t'' : dt.ITagOf S Sh P, (∀ i, t'' ≠ PfpTag.arg i) → t'' ≤ t) ⊓
...
noncomputable def blankMark : UMarkDef dt S Sh P where
  ...
  marked _ _ := True
  form _ t' := sideF _ (t' = PfpTag.sym) ⊓ canonF 0 (argVar 2 dt.dd 1)
  spec e t t' v := by
...
noncomputable abbrev wideStr :
  ...
  (dt.pfpInterp hpl hdef hacc p₀ mk).mapStructure e.α
...
theorem relMap_one {R : Language.wide.Relations 1}
    ...
    (hR : (dt.pfpInterp hpl hdef hacc p₀ mk).relFormula R = fun t => φ (t 0))
    (x : Univ e.α (RTagOf S Sh) P dt.KIx dt.dd) :
    RelMap R ![x] ↔ (φ x.1).Realize fun q => x.2 q.2 := by
  have h : RelMap R ![x] ↔
      ((dt.pfpInterp hpl hdef hacc p₀ mk).relFormula R fun i => (![x] i).1).Realize
...
theorem relMap_two {R : Language.wide.Relations 2}
    {φ : (dt.ITagOf S Sh P) → (dt.ITagOf S Sh P) →
      (L.sum Language.order).Formula (Fin 2 × Fin dt.dd)}
    (hR : (dt.pfpInterp hpl hdef hacc p₀ mk).relFormula R = fun t => φ (t 0) (t 1))
    (x y : Univ e.α (RTagOf S Sh) P dt.KIx dt.dd) :
    RelMap R ![x, y] ↔
      (φ x.1 y.1).Realize fun z => (if z.1 = 0 then x.2 else y.2) z.2 := by
  have h : RelMap R ![x, y] ↔
      ((dt.pfpInterp hpl hdef hacc p₀ mk).relFormula R fun i => (![x, y] i).1).Realize
