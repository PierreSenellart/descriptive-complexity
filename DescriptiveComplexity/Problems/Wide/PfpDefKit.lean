...
The first floor of the discharge `DescriptiveComplexity.Problems.Wide.PfpFactor`
sets up: each *kit* – the small, separately checkable composites the EXPSPACE
program's rule set is a sum of – meets
`DescriptiveComplexity.Pfp.URuleDefinable` for every one of its rule families,
...
alone, writing at most one track slot – `DescriptiveComplexity.Pfp.UTrDefinable.update`
