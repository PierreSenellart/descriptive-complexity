...
`DescriptiveComplexity.Problems.Wide.PfpAlphabet`. When every atom has been
...
* `DescriptiveComplexity.Pfp.qfValue` – the value of a formula given a truth value
  for each atom;
* `DescriptiveComplexity.Pfp.realize_qfValue` – on a quantifier-free formula it is
  the realization;
* `DescriptiveComplexity.Pfp.qfAtoms` – the atoms, finitely many;
* `DescriptiveComplexity.Pfp.qfValue_congr` – only those matter, so a machine that
...
realization is `DescriptiveComplexity.Pfp.qfValue` at the assignment that reads
