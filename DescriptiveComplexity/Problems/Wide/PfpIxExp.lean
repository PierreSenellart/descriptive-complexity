...
(`DescriptiveComplexity.Pfp.encTup`) in an argument block. At the elementwise
file that cell is an element of the universe
(`DescriptiveComplexity.Pfp.PfpData.expTagCell`/`expECell`); at a coarser one it
is a **register**, the one the layout names by the tuple's coordinates
(`DescriptiveComplexity.Pfp.PfpData.ixEncG_iff`), and the encoding's canonical
padding is what makes the two the same cell
(`DescriptiveComplexity.Pfp.PfpData.elt_reg_encCoord`).
...
(`DescriptiveComplexity.Pfp.PfpData.ixExp_run`). As in
`DescriptiveComplexity.Pfp.PfpData.ixStageAtom_reachesIn` the file is carried by
...
`DescriptiveComplexity.Pfp.PfpData.expLeaf`'s prefix takes over every wide
tuple, which `DescriptiveComplexity.Problems.Wide.PfpExp` reads as the
