...
(`DescriptiveComplexity.Pfp.PfpData.leafP`): every ∃-level of the register
...
`DescriptiveComplexity.Pfp.varRule` — which is generic in its `PX`/`SX`
...
variable {L : Language.{0, 0}} (dt : PfpData L) {A Q P PG PX SG SX : Type}
