...
* `DescriptiveComplexity.Pfp.PfpData.blkElt` – the element an encoded tuple
  is, in a given argument block, and `encCoord` – its coordinates, computed
  from the control's slots (the `cf` of
  `DescriptiveComplexity.Pfp.PfpData.nameGF`);
* `DescriptiveComplexity.Pfp.PfpData.encG_iff` – the leaf read's guard holds
...
* `DescriptiveComplexity.Pfp.PfpData.blk_encAsgTup_iff` and
  ...
  (`DescriptiveComplexity.Pfp.mem_encPt_asg`, `mem_encPt_tag`).
...
theorem regBit_blkElt (hlin : IsLinOrd (WMLe (A := Univ A R' P' dt.KIx dt.dd)))
    ...
    regBit m (wmSeg (dt.blkElt b v)) ↔
      wmBlk m (PfpTag.arg (toLex b) : PfpTag R' P' dt.KIx) v :=
...
This is `DescriptiveComplexity.Pfp.mem_encPt_asg` with the machine's cell in
place of the membership question. -/
theorem blk_encAsgTup_iff (hzo : zero ≠ one)
    (hlin : IsLinOrd (WMLe (A := Univ A R' P' dt.KIx dt.dd)))
    {m : Univ A R' P' dt.KIx dt.dd → Prop} {b : Fin dt.ko ⊕ Fin dt.ki}
    {p : dt.X.Map A}
    (hp : wmBlk m (PfpTag.arg (toLex b) : PfpTag R' P' dt.KIx) =
...
(`DescriptiveComplexity.Pfp.decRho`), which is what its domain
...
theorem regBit_decRho (hlin : IsLinOrd (WMLe (A := Univ A R' P' dt.KIx dt.dd)))
    ...
    regBit m (wmSeg (dt.blkElt b (encAsgTup dt.ly zero one i w))) ↔
      decRho dt.ly zero one
        (wmBlk m (PfpTag.arg (toLex b) : PfpTag R' P' dt.KIx)) i w :=
