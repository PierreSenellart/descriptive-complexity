...
`DescriptiveComplexity.Pfp.PfpData.expArgs`. The witness chain reads each
...
* `DescriptiveComplexity.Pfp.PfpData.expTagCell`/`expECell` – the cells the
...
* `DescriptiveComplexity.Pfp.PfpData.readLvE_expFam` – the loop element is
...
`DescriptiveComplexity.Pfp.PfpData.expECell`. -/
...
`DescriptiveComplexity.Pfp.PfpData.expLeafVal_iff`, hence of the exit
verdict. -/
