...
`DescriptiveComplexity.Pfp.exists_prenexPack` puts every formula into the prefix
...
content is `DescriptiveComplexity.Pfp.altQuantFrom_liftLast` – a prefix over a
...
right after it, both read off `DescriptiveComplexity.Pfp.ptFin`. -/
...
/-- **A source packed into a `DescriptiveComplexity.Pfp.PfpData`**: the prenex
...
chooses. -/
noncomputable def PfpData.ofSource (X : ExpExpansion L)
...
theorem PfpData.ki_pos (X : ExpExpansion L) (d : StepDef (X.E.sum Language.order))
    {dd : ℕ} (hdd : encDim X ≤ dd) : 0 < (PfpData.ofSource X d hdd).ki :=
  lt_of_lt_of_le (Nat.succ_pos _) ((PfpData.ofSource X d hdd).nOf_le_ki none)
