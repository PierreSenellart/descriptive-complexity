...
* `tgtTop` – load TARGET with the logical-top pattern
  (`DescriptiveComplexity.Pfp.MapKit`); exit into the startup seek.
* `seek1` – seek the marker to the logical top
  (`DescriptiveComplexity.Pfp.SeekKit`); its exit plants the permanent `ltp`
...
* `sweepAdv` – one round of the sweep
  (`DescriptiveComplexity.Pfp.AdvKit`); its landing exit re-enters the
...
* `compare` – the convergence test
  (`DescriptiveComplexity.Pfp.FlagSweepKit`); at the `ltp` cell its verdict
...
* `homeCmp`/`copy`/`homeCopy` – walk home, copy the next stage over the
  current one (`DescriptiveComplexity.Pfp.WriteSweepKit`, its first step in
...
Everything here is rules and shapes; the runs are `PfpRun`'s.
-/
...
variable {L : Language.{0, 0}} (dt : PfpData L) {A Q PE SE : Type}
...
in-shape separation, ready for `DescriptiveComplexity.Pfp.Assembly.prog` and
