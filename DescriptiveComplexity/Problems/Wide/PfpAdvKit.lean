...
The kit wrapping `DescriptiveComplexity.Pfp.Prog.reaches_advance` – the round
...
variable (hv' : WMSetLt WMLe v' (wmSeg gbot))
variable {m m' : Univ A R P K dd → Prop} (him : WMIncr WMLe m m')
...
variable (hrlB : ∀ r, restB r κ.rl = bitVal PR.zero PR.one (r = wmSeg gtop))
variable {p₀ : P} {fc : Q → A}
...
include hrules hR hlin hnerg hnerl hnewk htop hbot hi hv' him hAN hNB hNwk hNoth
  hwkB hrgB hrlB in
...
incremented. -/
theorem reaches
    (h₀ : PR.HasRight p₀ fc (PR.passTracks κ.t restA m v) (κ.emb .a1) fc
    ...
    Relation.ReflTransGen (wideData (Univ A R P K dd)).Step
      ...
      ⟨Sum.inr (PR.stElt (κ.emb .a4) fc), Sum.inl v',
        wideTape (PR.trackTape κ.t restB m') (PR.syElt PR.blank)⟩ := by
  have hlinSet := isLinOrd_wmSetLe (α := Univ A R P K dd) hlin
  -- the marker write's symbol equation
  have hput : Function.update (PR.passTracks κ.t restN m v') κ.wk PR.one =
      PR.passTracks κ.t restB m v' := by
    ...
    · rw [Function.update_of_ne hs]
      by_cases hst : s = κ.t
      · subst hst
        change (if κ.t = κ.t then bitVal PR.zero PR.one (regBit m v') else restN v' κ.t) =
          (if κ.t = κ.t then bitVal PR.zero PR.one (regBit m v') else restB v' κ.t)
        rw [if_pos rfl, if_pos rfl]
      · rw [Prog.passTracks_of_ne (show s ≠ κ.t from hst),
          Prog.passTracks_of_ne (show s ≠ κ.t from hst)]
        exact (hNoth s hs).symm
  have h₁ : PR.HasRight (κ.emb .a1) fc (PR.passTracks κ.t restN m v') (κ.emb .a2) fc
      (Function.update (PR.passTracks κ.t restN m v') κ.wk PR.one) :=
    (has_of_rule hrules (ρ := .put1) trivial).1 trivial
  rw [hput] at h₁
  -- the marker slot, read at or above the register file
  have hwkOff : ∀ (k r : Univ A R P K dd → Prop),
      (∃ x : Univ A R P K dd, WMSetLe WMLe (wmSeg x) r) →
      PR.passTracks κ.t restB k r κ.wk ≠ PR.one := by
    intro k r hbnd
    obtain ⟨x, hx⟩ := hbnd
    have hne : r ≠ v' := by
      rintro rfl
  ...
  have hrgOff : ∀ (k r : Univ A R P K dd → Prop),
      (∀ x : Univ A R P K dd, r ≠ wmSeg x) →
      PR.passTracks κ.t restB k r κ.rg ≠ PR.one := by
    intro k r hno
    rw [Prog.passTracks_of_ne (Ne.symm hnerg), hrgB,
      bitVal_neg fun hc => hc.elim fun x hx => hno x hx]
    exact PR.zero_ne_one
  exact Prog.reaches_advance hR hlin hnerg hnerl hnewk htop hbot hi hv' him
    ...
    ((has_of_rule hrules (ρ := .b1)
      (show PR.passTracks κ.t restB m (wmSeg gtop) κ.rl = PR.one by
    ...
    (fun _g hg => (has_of_rule hrules (ρ := .stay) (Or.inr hg)).2 not_false)
