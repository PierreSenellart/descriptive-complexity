...
* the embedding hypotheses are `DescriptiveComplexity.Pfp.IxSeg`;
* `DescriptiveComplexity.Pfp.exists_carry_ix` – an increment of the address
...
* `DescriptiveComplexity.Pfp.foldFrom_carry_of_ix` /
  `foldFrom_above_of_ix` / `foldFrom_below_of_ix` – the three fold rules, at
  the family `DescriptiveComplexity.Pfp.ixBlk` of image blocks;
* `DescriptiveComplexity.Pfp.foldFrom_bot_of_ix` /
