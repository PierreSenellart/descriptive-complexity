...
(`DescriptiveComplexity.Pfp.StageArgs`, `TagArgs`, `ElemArgs`), so what they
...
(`DescriptiveComplexity.Pfp.UConst`), since they are what the emitted formula
...
(`DescriptiveComplexity.Pfp.PfpData.kindRule`) is definable one constructor at
a time: the stage atom's by
`DescriptiveComplexity.Pfp.PfpData.uRulesDefinable_stageRule`, an expansion
...
variable {dt : PfpData L} [Fintype dt.SlotIx]
