...
`DescriptiveComplexity.Draw.withBit` writes a track and
`DescriptiveComplexity.Draw.bitAt` reads one, and the lemmas below are everything a
...
`DescriptiveComplexity.Draw.withBit_inj` and the form a distinctness obligation
comes in. -/
