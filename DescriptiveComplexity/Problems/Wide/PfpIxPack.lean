...
`DescriptiveComplexity.Pfp.PfpData.ixSpineSem`, at the states the VAL loop's
...
The branch `DescriptiveComplexity.Pfp.PfpData.ixGatedAt` takes is not merely
...
`DescriptiveComplexity.Pfp.PfpData.ixGatedSem₀` — the hypothesis the VAL
...
`DescriptiveComplexity.Pfp.PfpData.wellShapedG_ixBack_iff`. -/
      ...
      ∀ ℓ : Fin (dt.arOf (dt.varAt j)),
        IsEnc dt.ly PR.zero PR.one
          (wmBlk (ixAddr elt st.mir)
            (PfpTag.arg (toLex ((Sum.inl
              (Fin.castLE (dt.arOf_le_ko (dt.varAt j)) ℓ) :
              Fin dt.ko ⊕ Fin dt.ki))) : PfpTag R P dt.KIx)) := by
  classical
...
`DescriptiveComplexity.Pfp.PfpData.igVerdict_iff_isEnc`, which asks only that
...
theorem ixIGPassP_iff_isEnc
  ...
  · rintro ⟨p, hp⟩
    ...
    · refine (wellShapedIG_ixBack_iff (lay := F.toLayout) (elt := elt) hcellinj
      ...
      intro hblk hval
      have htag : (elt u).1 = (PfpTag.arg (toLex (dt.igBlk vi ℓ)) : PfpTag R P dt.KIx) :=
        (tagBlk_eq_some_iff _ _).mp hblk
      have hmem : encMap dt.ly PR.zero PR.one p ((elt u).2) := by
        rw [← hp]
        change ixAddr elt stV.val
          ((PfpTag.arg (toLex (dt.igBlk vi ℓ)) : PfpTag R P dt.KIx), (elt u).2)
        rw [← htag]
