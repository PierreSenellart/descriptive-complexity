...
This file mirrors `DescriptiveComplexity.Problems.Wide.PfpAtoms` at that
language: `DescriptiveComplexity.Pfp.BlkAtom` is the kind,
`DescriptiveComplexity.Pfp.blkAtom?` the total-on-atoms classifier,
`DescriptiveComplexity.Pfp.BlkAtom.holds` the semantic reading at a block
assignment, and
`DescriptiveComplexity.Pfp.realize_iff_qfValue_blkHolds` the capstone: a
