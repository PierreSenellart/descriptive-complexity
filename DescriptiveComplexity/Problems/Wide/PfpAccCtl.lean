...
`DescriptiveComplexity.Problems.Wide.PfpAcc` says what the machine must
...
`DescriptiveComplexity.Pfp.PfpData.varRule` are:

* `initSt` is `DescriptiveComplexity.Pfp.PfpData.initAcc` – every
  accumulator the polarity's unit, which is what an empty register's fold is
  (`accCVal_bot`);
* `postFold` is `DescriptiveComplexity.Pfp.PfpData.setLeaf` at `postLeaf` –
  the matrix's Boolean value at the atoms' verdicts, in the leaf flag;
* `storeCarry b` is `DescriptiveComplexity.Pfp.PfpData.carryAcc` – untouched
  below the carry, absorbing the chain at it, reset above (`accCVal_step`);
* `accBit` is `DescriptiveComplexity.Pfp.PfpData.accVerdict` – the chain from
...
fold's own correctness is `PfpAcc`'s; nothing here repeats it.
-/
...
level, which is `DescriptiveComplexity.Pfp.accCVal_bot`'s value. -/
...
`DescriptiveComplexity.Pfp.accCVal_step` asks for. -/
...
the atoms' readings – `DescriptiveComplexity.Pfp.realize_iff_qfValue_holds`'s
