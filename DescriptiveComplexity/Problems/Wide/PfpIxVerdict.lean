...
`DescriptiveComplexity.Problems.Wide.PfpVerdict` read at a coarse file: the
...
input of `DescriptiveComplexity.Pfp.PfpData.ixAccVerdict_varFM_qfValue`
discharged — the flags by
`DescriptiveComplexity.Pfp.PfpData.ixCtlBit_roundFX_pass_iff`, the pass by
`DescriptiveComplexity.Pfp.PfpData.ixRoundPass_of_polarities`, the valuation
and its pack by `DescriptiveComplexity.Pfp.PfpData.ixPassW`, the stage reads
by `DescriptiveComplexity.Pfp.PfpData.ixOld_stage_of_dict`, and the two
leaf readings by `DescriptiveComplexity.Pfp.PfpData.ixLeafP_pass_iff` /
`DescriptiveComplexity.Pfp.PfpData.ixLeafP_fail_iff`. -/
...
prefix of `DescriptiveComplexity.Pfp.PfpData.ixAccVerdict_leafP` read through
...
`DescriptiveComplexity.Pfp.PfpData.ixAccVerdict_leafP` read through
