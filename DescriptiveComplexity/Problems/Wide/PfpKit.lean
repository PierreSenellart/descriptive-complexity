...
* a *phase shape* – a small inductive, one constructor per phase the composite
  visits (`DescriptiveComplexity.Pfp.ReadPh`);
* a *rule shape* – one constructor per rule family, each with its concrete
  guard (`DescriptiveComplexity.Pfp.ReadRule`);
* the *rules function* – the shape's `DescriptiveComplexity.Pfp.Rule`s, at a
  phase embedding and the kit's slots
  (`DescriptiveComplexity.Pfp.ReadKit.rule`);
* a *discharge* – any program whose rule set contains the kit's rules
  ...
  holds for it (`DescriptiveComplexity.Pfp.ReadKit.reaches_pos`/`_neg`);
...
phase of a rule names its kit, so `DescriptiveComplexity.Pfp.Prog.sep_of`'s
hypothesis never crosses kits.

This file builds the smallest kit with full content – the named-bit read of
`DescriptiveComplexity.Problems.Wide.PfpRead` – as the template the others
follow.
-/
...
`DescriptiveComplexity.Pfp.Prog.sep_of` receives from an assembly: the
