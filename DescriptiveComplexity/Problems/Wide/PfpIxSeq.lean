...
`DescriptiveComplexity.Problems.Wide.PfpInstSeq` read at a coarse file: the
...
noncomputable def ixKindEndSt (dt : PfpData L) {A R P I : Type}
...
width. -/
noncomputable def ixKindCost (dt : PfpData L) (A : Type)
...
`DescriptiveComplexity.Pfp.PfpData.ixKind_hStage_thread` with the atom's cost
counted – `DescriptiveComplexity.Pfp.PfpData.ixKindCost`, whatever the kind. -/
...
`DescriptiveComplexity.Pfp.PfpData.ixKind_hStage` with the atom's cost counted –
`DescriptiveComplexity.Pfp.PfpData.ixKindCost`, whatever the kind. -/
...
the file's widths. -/
noncomputable def ixMatCost (dt : PfpData L) (A : Type) (vi : dt.VarIx)
    (w wP wR wK : ℕ) : ℕ :=
...
`DescriptiveComplexity.Pfp.PfpData.ixMatrix_run_thread` with the atoms
...
`DescriptiveComplexity.Pfp.PfpData.ixMatrix_run` with no boundary discipline
assumed — the tape ends in the threaded state
`DescriptiveComplexity.Pfp.PfpData.ixMatSt`, which differs from the entry
...
`DescriptiveComplexity.Pfp.PfpData.ixMatrix_run` with the atoms counted – one
...
noncomputable def ixGateCost (dt : PfpData L) (A : Type) (w wP : ℕ) : ℕ :=
...
variable (htagOf : ∀ ℓ : Fin (dt.arOf vi),
  dt.dspTagOf PR.zero PR.one
    (wmBlk (ixAddr elt st.mir) (PfpTag.arg (toLex (bOf ℓ)) : PfpTag R P dt.KIx)) =
  tOf ℓ)
...
`DescriptiveComplexity.Pfp.PfpData.ixGates_run` with the blocks counted – one
...
`DescriptiveComplexity.Pfp.PfpData.ixGates_run_fail` with the passing prefix
