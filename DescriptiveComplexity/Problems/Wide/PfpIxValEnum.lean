...
`DescriptiveComplexity.Pfp.PfpData.exists_valEnum` builds the chain the VAL loop
...
`DescriptiveComplexity.Pfp.PfpData.ixHolds_blkLaid`.
-/
...
`DescriptiveComplexity.Pfp.PfpData.exists_valEnum` gives over the elements, read
...
general chain at `DescriptiveComplexity.Pfp.PfpData.blkLaid`, whose four
...
`DescriptiveComplexity.Pfp.PfpData.nexTotal_lt_two_pow'` compares, the first
being the width (`DescriptiveComplexity.Pfp.PfpData.ixLegWidth_le`). -/
