...
theorem varGates_run (hTestOf : ∀ ℓ u, TestOf ℓ u) (f₀ : dt.CtlIx → A) :
    Relation.ReflTransGen (wideData (Univ A R P dt.KIx dt.dd)).Step
      ...
      ⟨Sum.inr (PR.stElt (emb .vchk1)
          (dt.gatesFs PR.zero PR.one vi st v
  ...
  have hwkS : ∀ r, dt.back PR.zero PR.one dt.dd0Le st r Slot.wk =
    ...
    intro r
    rw [back_wk, hwkSt]
  have hwkv' : PR.passTracks Slot.val (dt.back PR.zero PR.one dt.dd0Le st)
  ...
  refine Relation.ReflTransGen.trans
    (dt.gates_run (fun ℓ => Sum.inl (Fin.castLE (dt.arOf_le_ko vi) ℓ))
      dt.card_le_ntgDim
      (fun t => le_trans (Finset.le_sup
        (f := fun t' : dt.X.Tag => (dt.domPk t').n)
        (Finset.mem_univ t)) dt.domDepth_le_eDim)
      ...
      hgrules hR hlin hord htop hbot hv hvi hwkSt
...
theorem varGatesFail_run (ℓ₀ : Fin (dt.arOf vi))
    ...
    Relation.ReflTransGen (wideData (Univ A R P dt.KIx dt.dd)).Step
      ⟨Sum.inr (PR.stElt (emb (.gatesP (.chk 0))) f₀), Sum.inl v',
        wideTape (PR.trackTape Slot.val
          (dt.back PR.zero PR.one dt.dd0Le st) st.val) (PR.syElt PR.blank)⟩
      ⟨Sum.inr (PR.stElt (emb .vchk1)
          (dt.failCtl (PR := PR) (v := v) vi st tOf ℓ₀ f₀)), Sum.inl v,
        wideTape (PR.trackTape Slot.val
          (dt.back PR.zero PR.one dt.dd0Le st) st.val)
          (PR.syElt PR.blank)⟩ := by
  classical
  ...
  have hgrules : ∀ (i : dt.GatesSite vi) (ρ : dt.GatesSh vi i),
    ...
    fun i ρ => hrules (.gates i) ρ
  refine Relation.ReflTransGen.trans
    (dt.gates_run_fail (fun ℓ => Sum.inl (Fin.castLE (dt.arOf_le_ko vi) ℓ))
      dt.card_le_ntgDim
      (fun t => le_trans (Finset.le_sup
        (f := fun t' : dt.X.Tag => (dt.domPk t').n)
        (Finset.mem_univ t)) dt.domDepth_le_eDim)
...
checkpoint. -/
theorem varMatrix_run (stV : TapeSt dt A R P)
    ...
    Relation.ReflTransGen (wideData (Univ A R P dt.KIx dt.dd)).Step
      ...
      ⟨Sum.inr (PR.stElt (emb .mchk1)
          (dt.matFs PR.zero PR.one vi stV v
...
omit [Nonempty A] [L.IsRelational] [L.Structure A] hrules hR hv hwkSt in
...
and the order facts. -/
theorem varBg_back (stV : TapeSt dt A R P)
    (hwkV : stV.wk = fun r => r = v) :
    dt.VarBg PR.zero PR.one v gtop
      (dt.back PR.zero PR.one dt.dd0Le stV) stV.val := by
  refine ⟨fun r => ?_, fun r => rfl,
    fun r => dt.back_regLast hlin htop hord r,
    fun u b => dt.back_blk_wmSeg hlin u b, fun r => rfl⟩
  rw [back_wk, hwkV]
...
theorem varMachineFail_run (ℓ₀ : Fin (dt.arOf vi))
    ...
    (hoff : ∀ r, r ≠ v → rest' r = dt.back PR.zero PR.one dt.dd0Le st r)
  ...
  have hflagF : dt.failCtl (PR := PR) (v := v) vi st tOf ℓ₀
      ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
        (dt.back PR.zero PR.one dt.dd0Le st v))
        ...
        (dt.setCtl PR.zero PR.one dt.gateFlagC False
          ((dt.varArgsOf PR.zero PR.one vi).enterBlockSt ℓ₀
            (dt.gatesFs PR.zero PR.one vi st v
              ...
              ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
                (dt.back PR.zero PR.one dt.dd0Le st v)) (ℓ₀ : ℕ))
            (dt.back PR.zero PR.one dt.dd0Le st v)))
        dt.gateFlagC := hc
    exact (ctlBit_setCtl_self hzo _ _ _).mp hc'
  exact dt.var_run_fail (fun i ρ => hrules i ρ) hR hlin hbot hv hvi
    (dt.varBg_back hlin hord htop st hwkSt)
    (dt.varGatesFail_run vi st hrules hR hlin hord htop hbot hv hvi hwkSt
...
theorem varMachineUngated_run (hTestOf : ∀ ℓ u, TestOf ℓ u)
    ...
    Relation.ReflTransGen (wideData (Univ A R P dt.KIx dt.dd)).Step
      ⟨Sum.inr (PR.stElt (emb .vchk0) f₀), Sum.inl v,
        wideTape (PR.trackTape Slot.val
          (dt.back PR.zero PR.one dt.dd0Le st) st.val) (PR.syElt PR.blank)⟩
      ⟨Sum.inr (PR.stElt exitPh
          (dt.ungatedCtl (PR := PR) (v := v) vi st tOf
            ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
              (dt.back PR.zero PR.one dt.dd0Le st v)))), Sum.inl v',
        wideTape (PR.trackTape Slot.val rest' st.val)
          (PR.syElt PR.blank)⟩ := by
  ...
  have hns : (dt.varArgsOf PR.zero PR.one vi).newSlot ≠ Slot.val := by
    rcases vi with _ | i <;> exact fun h => nomatch h
  have hflagF : dt.ungatedCtl (PR := PR) (v := v) vi st tOf
      ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
        (dt.back PR.zero PR.one dt.dd0Le st v))
      (dt.varArgsOf PR.zero PR.one vi).gateFlag ≠ PR.one := by
    intro hcx
    have hchar := dt.ctlBit_gateFlagC_gatesFs (st := st) (v := v)
      ...
      (tOf := tOf)
      (enterSt := (dt.varArgsOf PR.zero PR.one vi).enterBlockSt)
      hzo (fun ℓ f g => Iff.rfl)
      ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
        (dt.back PR.zero PR.one dt.dd0Le st v)) (dt.arOf vi)
  ...
  exact dt.var_run_fail (fun i ρ => hrules i ρ) hR hlin hbot hv hvi
    (dt.varBg_back hlin hord htop st hwkSt)
    (dt.varGates_run vi st hrules hR hlin hord htop hbot hv hvi hwkSt
      TestOf hcompatOf tOf htagOf hTestOf
      ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
        (dt.back PR.zero PR.one dt.dd0Le st v)))
    hflagF hns hoff hupd
...
theorem roundSt_val (stV : TapeSt dt A R P) : dt.roundSt stV stV.val = stV :=
  rfl
...
theorem back_roundSt_off {zero one : A}
    ...
    (hs : s ≠ Slot.val) :
    dt.back zero one dt.dd0Le (dt.roundSt st m₁) r s =
      dt.back zero one dt.dd0Le (dt.roundSt st m₂) r s := by
  match s with
...
noncomputable def roundFX (q : dt.CtlIx → A) (a : ιV) : dt.CtlIx → A :=
  dt.roundCtl PR.zero_ne_one vi (dt.roundSt st (mV a)) v (semOf a) q
...
theorem readAcc_varFM
    ...
    (a : ιV) (j : ℕ) (hj : j < dt.ki) :
    dt.readAcc PR.one (dt.varFM vi st v mV semOf fG a) j ↔
  ...
  have hride : ∀ (q : dt.CtlIx → A) (a' : ιV) (i : ℕ),
      dt.readAcc PR.one ((dt.varArgsOf PR.zero PR.one vi).postFold
        (dt.roundFX vi st v mV semOf q a')
        (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV a')) v)) i ↔
      dt.readAcc PR.one q i := by
    intro q a' i
    have h2 : ∀ jj : Fin dt.naDim,
        dt.roundFX vi st v mV semOf q a' (dt.accC jj) = q (dt.accC jj) :=
      fun jj => dt.roundCtl_apply_accC vi (dt.roundSt st (mV a')) v
        (semOf a') q jj
    have h1 : dt.readAcc PR.one ((dt.varArgsOf PR.zero PR.one vi).postFold
        (dt.roundFX vi st v mV semOf q a')
        (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV a')) v)) i ↔
        dt.readAcc PR.one (dt.roundFX vi st v mV semOf q a') i :=
  ...
  have hleafride : ∀ (q : dt.CtlIx → A) (a' : ιV),
      dt.ctlBit PR.one ((dt.varArgsOf PR.zero PR.one vi).postFold
        ...
        (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV a')) v))
        dt.leafC ↔
      dt.roundLeaf (PR := PR) vi (dt.roundFX vi st v mV semOf q a') :=
    ...
    have hstep' := iterOrd_covers
      ...
      (step := fun a q =>
        (dt.varArgsOf PR.zero PR.one vi).storeCarry
          ...
          (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV a)) v))
      hwz hnb
    rw [show dt.varFM vi st v mV semOf fG z = _ from hstep', hcarry]
    -- the previous round's vector and leaf
    have hacc : ∀ i : ℕ, i < dt.ki →
        (dt.readAcc PR.one ((dt.varArgsOf PR.zero PR.one vi).postFold
          (dt.roundFX vi st v mV semOf (dt.varFM vi st v mV semOf fG w) w)
          (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV w)) v)) i ↔
    ...
    have hnew : ∀ i : ℕ, i < dt.ki →
        (dt.readAcc PR.one
          ((dt.varArgsOf PR.zero PR.one vi).storeCarry
            ...
            ((dt.varArgsOf PR.zero PR.one vi).postFold
              (dt.roundFX vi st v mV semOf
                (dt.varFM vi st v mV semOf fG w) w)
              (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV w)) v))
            (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV w)) v)) i ↔
        (if i < (c : ℕ) then
            dt.readAcc PR.one
...
theorem accVerdict_varFM
  ...
  have hride : ∀ i : ℕ,
      dt.readAcc PR.one ((dt.varArgsOf PR.zero PR.one vi).postFold
        (dt.roundFX vi st v mV semOf (dt.varFM vi st v mV semOf fG aT) aT)
        (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV aT)) v)) i ↔
      dt.readAcc PR.one (dt.varFM vi st v mV semOf fG aT) i := by
    intro i
    have h2 : ∀ jj : Fin dt.naDim,
        dt.roundFX vi st v mV semOf (dt.varFM vi st v mV semOf fG aT) aT
          (dt.accC jj) =
        dt.varFM vi st v mV semOf fG aT (dt.accC jj) :=
      fun jj => dt.roundCtl_apply_accC vi (dt.roundSt st (mV aT)) v
        (semOf aT) _ jj
  ...
  have hfold := dt.accVerdict_iff_foldFrom
    ...
    (fun j hj => (hride j).trans
      (readAcc_varFM vi st hlin hord mV semOf hbotV hmV0 hIncr hKin fG
        hleaf aT j hj))
...
theorem postLeaf_roundFX [LinearOrder (dt.X.Map A)]
    ...
    dt.postLeaf PR.one vi
      (dt.matFs PR.zero PR.one vi (dt.roundSt st (mV a)) v
  ...
  exact dt.ctlBit_avC_matFs_holds PR.zero_ne_one hlin σ wa hENCa hOlda
...
theorem accVerdict_varFM_qfValue [LinearOrder (dt.X.Map A)]
  ...
  have hleaf : ∀ a : ιV, dt.roundLeaf (PR := PR) vi
    ...
    have hExig : dt.ctlBit PR.one
      ...
      rw [show dt.roundFX vi st v mV semOf
        ...
        ctlBit, ctlBit,
        dt.roundCtl_apply_roundFlag (Or.inl rfl) vi (dt.roundSt st (mV a))
          v (semOf a) (dt.varFM vi st v mV semOf fG a)]
    have hAlig : dt.ctlBit PR.one
      ...
      rw [show dt.roundFX vi st v mV semOf
        ...
        dt.roundCtl PR.zero_ne_one vi (dt.roundSt st (mV a)) v (semOf a)
          (dt.varFM vi st v mV semOf fG a) from rfl,
        ctlBit, ctlBit,
        dt.roundCtl_apply_roundFlag (Or.inr rfl) vi (dt.roundSt st (mV a))
          v (semOf a) (dt.varFM vi st v mV semOf fG a)]
    rw [roundLeaf]
    ...
    · -- a passing round: the leaf is the matrix's value at its points
      ...
      have hmat : dt.ctlBit PR.one
        ...
        rw [show dt.roundFX vi st v mV semOf
          ...
          dt.roundCtl PR.zero_ne_one vi (dt.roundSt st (mV a)) v (semOf a)
            (dt.varFM vi st v mV semOf fG a) from rfl,
          dt.roundCtl_of_flags vi (dt.roundSt st (mV a)) v (semOf a)
            (dt.varFM vi st v mV semOf fG a) hp (hExig.mp hex)
            (hAlig.mp hal),
          hsem']
      ...
      · rintro ⟨h1, h2⟩
        ...
        exact (dt.postLeaf_roundFX vi st hlin mV σ a (wOf a hp)
          (hENC a hp) (hOld a hp) hordP
          (dt.igFs PR.zero PR.one vi (dt.roundSt st (mV a)) v
            (dt.varFM vi st v mV semOf fG a) (dt.nIn vi))).mp h2'
      · rintro ⟨h1, h2⟩
        ...
        exact (dt.postLeaf_roundFX vi st hlin mV σ a (wOf a hp)
          (hENC a hp) (hOld a hp) hordP
          (dt.igFs PR.zero PR.one vi (dt.roundSt st (mV a)) v
            (dt.varFM vi st v mV semOf fG a) (dt.nIn vi))).mpr
          (h2 ((hAll a).mp hal))
  ...
  exact accVerdict_varFM vi st hlin hord mV semOf hbotV hmV0 hIncr hKin
    hTop fG hleaf
...
noncomputable def varRdSt (p : Scratch dt A R P)
    (m : Univ A R P dt.KIx dt.dd → Prop) : TapeSt dt A R P :=
...
noncomputable def varPairT (fG : dt.CtlIx → A) :
  ...
  iterOrd ((st.sav, st.tgt),
    ...
    (fun a p =>
      (((dt.roundEndSt PR.zero PR.one vi (dt.varRdSt st p.1 (mV a)) v p.2).sav,
        (dt.roundEndSt PR.zero PR.one vi (dt.varRdSt st p.1 (mV a)) v p.2).tgt),
        (dt.varArgsOf PR.zero PR.one vi).storeCarry
          (tagBlk (dt.valCarry (mV a)).1)
          ((dt.varArgsOf PR.zero PR.one vi).postFold
            (dt.roundCtlT PR.zero_ne_one vi (dt.varRdSt st p.1 (mV a)) v
              (semT p.1 a) p.2)
            (dt.back PR.zero PR.one dt.dd0Le
              (dt.roundEndSt PR.zero PR.one vi
...
noncomputable def varSTT (fG : dt.CtlIx → A) (a : ιV) : Scratch dt A R P :=
  (dt.varPairT vi st v mV semT fG a).1

variable (v) in
/-- The tape family of the VAL loop — the state each round is entered
in. -/
noncomputable def varStT (fG : dt.CtlIx → A) (a : ιV) : TapeSt dt A R P :=
  dt.varRdSt st (dt.varSTT vi st v mV semT fG a) (mV a)
...
noncomputable def varFMT (fG : dt.CtlIx → A) (a : ιV) : dt.CtlIx → A :=
  (dt.varPairT vi st v mV semT fG a).2

variable (v) in
/-- **The state one round ends in** — the next round's entry state at a
cover, and the loop's exit state at the top. -/
noncomputable def varStE (fG : dt.CtlIx → A) (a : ιV) : TapeSt dt A R P :=
  dt.roundEndSt PR.zero PR.one vi (dt.varStT vi st v mV semT fG a) v
...
noncomputable def varFXT (fG : dt.CtlIx → A) (a : ιV) : dt.CtlIx → A :=
  dt.roundCtlT PR.zero_ne_one vi (dt.varStT vi st v mV semT fG a) v
    (semT (dt.varSTT vi st v mV semT fG a) a)
    (dt.varFMT vi st v mV semT fG a)
...
theorem varStT_fields (fG : dt.CtlIx → A) (a : ιV) :
    (dt.varStT vi st v mV semT fG a).wk = st.wk ∧
      (dt.varStT vi st v mV semT fG a).mir = st.mir ∧
      (dt.varStT vi st v mV semT fG a).bot = st.bot ∧
      (dt.varStT vi st v mV semT fG a).old = st.old ∧
      (dt.varStT vi st v mV semT fG a).val = mV a :=
  ⟨rfl, rfl, rfl, rfl, rfl⟩

omit [Finite R] [Finite P] [Finite dt.KIx] [Finite ιV] in
/-- A round leaves the register it was entered with. -/
theorem varStE_val (fG : dt.CtlIx → A) (a : ιV) :
    (dt.varStE vi st v mV semT fG a).val = mV a :=
  (dt.roundEndSt_fields vi (dt.varStT vi st v mV semT fG a) v
...
theorem varRdSt_varStE (fG : dt.CtlIx → A) (a : ιV)
    ...
    dt.varRdSt st ((dt.varStE vi st v mV semT fG a).sav,
        (dt.varStE vi st v mV semT fG a).tgt) m =
      dt.roundSt (dt.varStE vi st v mV semT fG a) m := by
  conv_rhs => rw [varStE, dt.roundEndSt_eq vi (dt.varStT vi st v mV semT fG a)
    v (dt.varFMT vi st v mV semT fG a)]
  rfl
...
theorem varSTT_bot {a₀ : ιV} (hbotV : ∀ a : ιV, a₀ ≤ a)
    (fG : dt.CtlIx → A) :
    dt.varSTT vi st v mV semT fG a₀ = (st.sav, st.tgt) :=
...
theorem varStT_bot {a₀ : ιV} (hbotV : ∀ a : ιV, a₀ ≤ a)
    (fG : dt.CtlIx → A) :
    dt.varStT vi st v mV semT fG a₀ = dt.roundSt st (mV a₀) := by
  rw [varStT, dt.varSTT_bot vi st mV semT hbotV fG]
  rfl
...
theorem varStT_covers {a a' : ιV} (hlt : a < a')
    ...
    dt.varStT vi st v mV semT fG a' =
      dt.roundSt (dt.varStE vi st v mV semT fG a) (mV a') := by
  rw [varStT, show dt.varSTT vi st v mV semT fG a' = _ from
    congrArg Prod.fst (iterOrd_covers hlt hnb)]
  exact dt.varRdSt_varStE vi st mV semT fG a (mV a')
...
theorem varFMT_covers {a a' : ιV} (hlt : a < a')
    ...
    dt.varFMT vi st v mV semT fG a' =
      (dt.varArgsOf PR.zero PR.one vi).storeCarry
        (tagBlk (dt.valCarry (mV a)).1)
        ((dt.varArgsOf PR.zero PR.one vi).postFold
          (dt.varFXT vi st v mV semT fG a)
          (dt.back PR.zero PR.one dt.dd0Le
...
include hrules hR hlin hord htop hbot hv hvi hwkSt hcompatOf hbotV
...
theorem varMachine_run
  ...
  set fG := dt.gatesFs PR.zero PR.one vi st v
    (fun ℓ => Sum.inl (Fin.castLE (dt.arOf_le_ko vi) ℓ))
    dt.card_le_ntgDim
    (fun t => le_trans (Finset.le_sup
      (f := fun t' : dt.X.Tag => (dt.domPk t').n)
    ...
    tOf (dt.varArgsOf PR.zero PR.one vi).enterBlockSt
    ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
      (dt.back PR.zero PR.one dt.dd0Le st v)) (dt.arOf vi) with hfG
  ...
  refine dt.var_run (fun i ρ => hrules i ρ) hR hlin htop hbot hv hvi
    (dt.varBg_back hlin hord htop st hwkSt)
    (dt.varGates_run vi st hrules hR hlin hord htop hbot hv hvi hwkSt
      TestOf hcompatOf tOf htagOf hTestOf
      ((dt.varArgsOf PR.zero PR.one vi).enterSt f₀
        (dt.back PR.zero PR.one dt.dd0Le st v)))
    hflag
    (dt.varBg_back hlin hord htop (dt.roundSt st (fun _ => False)) hwkSt)
    (fun r s hs => dt.back_roundSt_off (st := st) _ _ r s hs)
    hbotV htopV hmV0
    (restO := fun a => dt.back PR.zero PR.one dt.dd0Le
      (dt.roundSt st (mV a)))
    (fun a => dt.varBg_back hlin hord htop (dt.roundSt st (mV a)) hwkSt)
    (restM := fun a => dt.back PR.zero PR.one dt.dd0Le
      (dt.roundSt st (mV a)))
    ?_
    (fM := dt.varFM vi st v mV semOf fG)
    (fX := fun a => dt.roundFX vi st v mV semOf
      (dt.varFM vi st v mV semOf fG a) a)
    ?_ ?_ hIncr ?_ ?_ hTestT hTestF
  · -- the first round's background is the cleared one
    exact congrArg (fun m => dt.back PR.zero PR.one dt.dd0Le
  ...
  · -- the round, per VAL content: the inner gates, the branch and the
    ...
    intro a
    exact dt.round_run vi hrules hR hlin hord htop hbot hv hvi
      (dt.roundSt st (mV a)) hwkSt hmir hbotSt hsav htgt (semOf a)
      (dt.varFM vi st v mV semOf fG a)
  · -- backgrounds agree across a cover, off VAL
    intro a a' hlt hnb r s hs
    exact dt.back_roundSt_off (st := st) _ _ r s hs
    ...
    have hcov := iterOrd_covers
      ...
      (step := fun a q =>
        (dt.varArgsOf PR.zero PR.one vi).storeCarry
          ...
          (dt.back PR.zero PR.one dt.dd0Le (dt.roundSt st (mV a)) v))
      hlt hnb
    rw [show dt.varFM vi st v mV semOf fG a' = _ from hcov,
      valCarry_eq hlin h1 h2]
    rfl
...
theorem varMachine_run_thread
    ...
    Relation.ReflTransGen (wideData (Univ A R P dt.KIx dt.dd)).Step
      ...
      ⟨Sum.inr (PR.stElt (emb .vchk2)
          ((dt.varArgsOf PR.zero PR.one vi).postFold
            (dt.varFXT vi st v mV semT
              (dt.varFG (PR := PR) vi st v tOf f₀) aT)
            (dt.back PR.zero PR.one dt.dd0Le
              (dt.varStE vi st v mV semT (dt.varFG (PR := PR) vi st v tOf f₀) aT) v))),
        Sum.inl v,
        wideTape (PR.trackTape Slot.val
          (dt.back PR.zero PR.one dt.dd0Le
            (dt.varStE vi st v mV semT (dt.varFG (PR := PR) vi st v tOf f₀) aT))
          (mV aT)) (PR.syElt PR.blank)⟩ := by
  classical
  have hzo := PR.zero_ne_one
  have htagOf : ∀ ℓ : Fin (dt.arOf vi),
      ...
      tOf ℓ :=
    fun ℓ => dspTagOf_eq_of_onehot (hwitOf ℓ)
  set fG := dt.varFG (PR := PR) vi st v tOf f₀ with hfG
  -- the verdict flag holds after the gates
  have hflag : fG dt.gateFlagC = PR.one := by
    rw [hfG, varFG]
    have hchar := dt.ctlBit_gateFlagC_gatesFs (st := st) (v := v)
  ...
  have hwkA : ∀ a : ιV, (dt.varStT vi st v mV semT fG a).wk = fun r => r = v :=
    fun a => ((dt.varStT_fields vi st mV semT fG a).1).trans hwkSt
  have hmirA : ∀ a : ιV, (dt.varStT vi st v mV semT fG a).mir = v :=
    fun a => ((dt.varStT_fields vi st mV semT fG a).2.1).trans hmir
  have hbotA : ∀ a : ιV, (dt.varStT vi st v mV semT fG a).bot =
      fun r => r = (fun _ => False) :=
    fun a => ((dt.varStT_fields vi st mV semT fG a).2.2.1).trans hbotSt
  have hvalE : ∀ a : ιV, (dt.varStE vi st v mV semT fG a).val = mV a :=
    fun a => dt.varStE_val vi st mV semT fG a
  refine dt.var_run (fun i ρ => hrules i ρ) hR hlin htop hbot hv hvi
    (dt.varBg_back hlin hord htop st hwkSt)
    (dt.varGates_run vi st hrules hR hlin hord htop hbot hv hvi hwkSt
      TestOf hcompatOf tOf htagOf hTestOf
    ...
    hbotV htopV hmV0
    (restO := fun a => dt.back PR.zero PR.one dt.dd0Le
      (dt.varStE vi st v mV semT fG a))
    (fun a => (hvalE a) ▸ dt.varBg_back hlin hord htop
      (dt.varStE vi st v mV semT fG a)
      (((dt.roundEndSt_fields vi (dt.varStT vi st v mV semT fG a) v
        (dt.varFMT vi st v mV semT fG a)).1).trans (hwkA a)))
    (restM := fun a => dt.back PR.zero PR.one dt.dd0Le
      (dt.varStT vi st v mV semT fG a))
    ?_
    (fM := dt.varFMT vi st v mV semT fG)
    (fX := dt.varFXT vi st v mV semT fG)
    ?_ ?_ hIncr ?_ ?_ hTestT hTestF
  · -- the first round's background is the cleared one
    rw [show dt.varStT vi st v mV semT fG a₀ = dt.roundSt st (mV a₀) from
      dt.varStT_bot vi st mV semT hbotV fG, hmV0]
  · -- the thread starts at the fold's start
    exact dt.varFMT_bot vi st mV semT hbotV fG
  · -- the round, at the state the thread produced for it
    intro a
    have hrun := dt.round_run_thread vi hrules hR hlin hord htop hbot hv hvi
      (dt.varStT vi st v mV semT fG a) (hwkA a) (hmirA a) (hbotA a)
      (semT (dt.varSTT vi st v mV semT fG a) a)
      (dt.varFMT vi st v mV semT fG a)
    rw [show (dt.roundEndSt PR.zero PR.one vi
        (dt.varStT vi st v mV semT fG a) v
        (dt.varFMT vi st v mV semT fG a)).val = mV a from hvalE a] at hrun
    exact hrun
  · -- backgrounds agree across a cover, off VAL
    intro a a' hlt hnb r s hs
    rw [show dt.varStT vi st v mV semT fG a' =
      dt.roundSt (dt.varStE vi st v mV semT fG a) (mV a') from
      dt.varStT_covers vi st mV semT hlt hnb fG]
    exact dt.back_roundSt_off (st := dt.varStE vi st v mV semT fG a)
      _ _ r s hs
  · -- the store at a cover is the thread's step
    intro a a' hlt hnb u₀ h1 h2
    rw [show dt.varFMT vi st v mV semT fG a' = _ from
      dt.varFMT_covers vi st mV semT hlt hnb fG, valCarry_eq hlin h1 h2]
