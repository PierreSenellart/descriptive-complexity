...
`DescriptiveComplexity.Pfp.Table` is indexed by two arbitrary types – `R`, the
rules, and `P`, the phases – which are the two halves of
`DescriptiveComplexity.Pfp.PfpTag`. A transition is therefore an element
...
(`DescriptiveComplexity.Pfp.Table.Reads`); the interpretation that makes the
...
| for | theorem |
|---|---|
| a rule fires, moving right | `DescriptiveComplexity.Pfp.Table.fire_right` |
| a rule fires, moving left | `DescriptiveComplexity.Pfp.Table.fire_left` |
| the promise of `WideAcceptSpace` | `DescriptiveComplexity.Pfp.Table.wellFormed` |
| the promise `DWideAcceptSpace` adds | `DescriptiveComplexity.Pfp.Table.deterministic` |
...
table, `DescriptiveComplexity.Pfp.Table.Sep`: two guarded rules agreeing on the
state they apply in and the symbol they read are the same rule with the same
data. The padding of `DescriptiveComplexity.Pfp.pad` is what makes that a
...
abbrev Univ (A R P K : Type) (dd : ℕ) : Type := PfpTag R P K × (Fin dd → A)
...
`DescriptiveComplexity.Pfp.unpad_pad` applied to the payload, and together they
...
theorem isLinOrd_wmLe (hR : T.Reads) : IsLinOrd (WMLe (A := Univ A R P K dd)) := by
  have h := Wide.isLinOrd_tagTupleLe (Tag := PfpTag R P K) (d := dd) (A := A)
...
`DescriptiveComplexity.Pfp.wmSetLe_logicalTop` is about the block-major order on
...
theorem wmSetLe_logicalTop_reads (hR : T.Reads) {s : Univ A R P K dd → Prop}
    (hjunk : ∀ τ : PfpTag R P K, (∀ i : K, τ ≠ PfpTag.arg i) → ∀ v : Fin dd → A, ¬s (τ, v)) :
...
Between them and `DescriptiveComplexity.Pfp.Table.fire_right` /
`DescriptiveComplexity.Pfp.Table.fire_left`, a program never mentions
