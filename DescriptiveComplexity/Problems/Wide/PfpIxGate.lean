...
`DescriptiveComplexity.Problems.Wide.PfpInstGate` read at a coarse file exactly
as `DescriptiveComplexity.Problems.Wide.PfpIxExp` reads the expansion atoms:
the cells become the registers the layout names
(`DescriptiveComplexity.Pfp.PfpData.ixEncG_iff`), the marks become marks on
...
theorem ctlBit_ixGateTagFam_wit (hzo : zero ≠ one) (f₀ : dt.CtlIx → A)
    ...
    dt.ctlBit one
        (dt.ixGateTagFam F hhas one b st hc hn hrd vAdr f₀
          (Fin.last (Fintype.card dt.X.Tag))) (dt.gateTagC hc t') ↔
      wmBlk (ixAddr elt st.mir) (PfpTag.arg (toLex b) : PfpTag R P dt.KIx)
...
holds. -/
noncomputable def ixGateLeafP (dt : PfpData L) {I : Type}
...
variable (htag : dt.dspTagOf PR.zero PR.one
  (wmBlk (ixAddr eltR st.mir) (PfpTag.arg (toLex b) : PfpTag R P dt.KIx)) = t)
      ...
      (dt.ctlBit one f₀ dt.gateFlagC ∧
        ...
        ExpExpansion.DomHolds (X := dt.X)
          (t, decRho dt.ly zero one
            (wmBlk (ixAddr elt st.mir) (PfpTag.arg (toLex b) : PfpTag R P dt.KIx)))) := by
