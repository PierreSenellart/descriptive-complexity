...
casts through the budgets that `DescriptiveComplexity.Pfp.PfpData.eDim` and
...
bit, and writing one (`DescriptiveComplexity.Pfp.PfpData.ctlBit` and
...
`DescriptiveComplexity.Pfp.tupNext` and friends, in the slots: where a loop
