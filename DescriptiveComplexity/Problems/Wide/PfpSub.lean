...
(`DescriptiveComplexity.Pfp.Prog.step_writeCell` and its rightward twin): the
...
`DescriptiveComplexity.Pfp.Prog.passTracks_update_cell` is the equation the
...
`DescriptiveComplexity.Pfp.Prog.reaches_fileTestG` restates it with the question a
