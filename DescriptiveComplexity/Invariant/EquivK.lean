...
(`DescriptiveComplexity.atomicAgree` – same equalities between coordinates,
same base relations at every selection of coordinates), and
`DescriptiveComplexity.EquivK (atomicAgree L A k)` is `k`-variable equivalence
`≡ᵏ` of `k`-tuples over the structure `A`.
...
relational vocabulary cannot separate `≡ᵏ`-equivalent tuples, when its free
variables read the tuples through an arbitrary selection of coordinates, its
bound variables are placed injectively outside that selection, and the
positions not in use leave room for its quantifier depth
...
/-- Agreement on the atomic type: two `k`-tuples agree on all equalities
between their coordinates and on all base relations at every selection of
coordinates. This is the initial relation of the `k`-pebble refinement over a
structure. -/
def atomicAgree (L : Language.{0, 0}) (A : Type) [L.Structure A] (k : ℕ) :
    PebbleRel A k :=
  fun v w =>
    (∀ i j : Fin k, v i = v j ↔ w i = w j) ∧
    ∀ {l : ℕ} (R : L.Relations l) (g : Fin l → Fin k),
      (RelMap R fun p => v (g p)) ↔ RelMap R fun p => w (g p)

/-- Agreement on the atomic type is an equivalence. -/
theorem atomicAgree_equivalence [L.Structure A] :
    Equivalence (atomicAgree L A k) where
  refl _ := ⟨fun _ _ => Iff.rfl, fun _ _ => Iff.rfl⟩
  symm h := ⟨fun i j => (h.1 i j).symm, fun R g => (h.2 R g).symm⟩
  trans h h' := ⟨fun i j => (h.1 i j).trans (h'.1 i j),
    fun R g => (h.2 R g).trans (h'.2 R g)⟩

/-- `≡ᵏ` over a structure is an equivalence. -/
theorem equivK_atomicAgree_equivalence [L.Structure A] :
    Equivalence (EquivK (atomicAgree L A k)) :=
  equivK_equivalence atomicAgree_equivalence
...
private theorem snoc_injective (hinj : Function.Injective h)
...
private theorem image_snoc_subset :
...
variable [L.IsRelational] [L.Structure A] [Finite A]

/-- **The `k`-variable invariance lemma.** A first-order formula over a
relational vocabulary cannot separate `≡ᵏ`-equivalent `k`-tuples: its free
variables read the tuples through an arbitrary selection `g` of coordinates,
its bound variables through an injective selection `h` disjoint from `g`, and
the coordinates not in use leave room for its quantifier depth. Each
quantifier spends one fresh pebble, placed by the game move
(`DescriptiveComplexity.EquivK.update`); atomic formulas are decided by the
initial agreement (`DescriptiveComplexity.atomicAgree`). -/
theorem realize_equivK {m : ℕ} :
    ∀ {n : ℕ} (φ : L.BoundedFormula (Fin m) n) (g : Fin m → Fin k)
      ...
      ∀ v w : Fin k → A, EquivK (atomicAgree L A k) v w →
      ((φ.Realize (fun i => v (g i)) fun j => v (h j)) ↔
        φ.Realize (fun i => w (g i)) fun j => w (h j)) := by
  intro n φ
  induction φ with
  | falsum => exact fun _ _ _ _ _ _ _ _ => Iff.rfl
  | @equal n t₁ t₂ =>
    intro g h _ _ _ v w hvw
  ...
  | @rel n l R ts =>
    ...
    exact Iff.trans (iff_of_eq (congrArg (RelMap R) (key v)))
      (Iff.trans (hvw.initial.2 R fun j => Sum.elim g h (x j))
        (iff_of_eq (congrArg (RelMap R) (key w))).symm)
  | @imp n f₁ f₂ ih₁ ih₂ =>
    intro g h hinj hdisj hroom v w hvw
  ...
  | @all n ψ ih =>
    ...
    · intro hall b
      ...
      have hih := ih g (Fin.snoc h p) hinj' hdisj' hroom'
...
relational vocabulary, with room for its quantifier depth beyond the
selection of coordinates its free variables read, cannot separate
`≡ᵏ`-equivalent tuples. -/
theorem realize_formula_equivK {m : ℕ} (φ : L.Formula (Fin m)) (g : Fin m → Fin k)
    (hroom : (Finset.image g Finset.univ).card + qdepth φ ≤ k)
    {v w : Fin k → A} (hvw : EquivK (atomicAgree L A k) v w) :
