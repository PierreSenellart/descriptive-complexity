...
(`FirstOrder.FOInterpretation.comp`), and hence FO reducibility is transitive
(`FirstOrder.FOReduction.trans`): from `P ≤ᶠᵒ Q` and `Q ≤ᶠᵒ R` one gets
`P ≤ᶠᵒ R`.
...
of `J` through `I` (`FirstOrder.FOInterpretation.pull`): atoms become the
...
* the composite's universe `(Tag₂ × (Fin d₂ → Tag₁)) × A^(d₂·d₁)` is only
  *isomorphic* (`FirstOrder.FOInterpretation.compLEquiv`), not equal, to the
