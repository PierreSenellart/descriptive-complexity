...
* `FirstOrder.ncard_image_equiv`: invariance of the decoded number under
  equivalences (this is what feeds `DecisionProblem.iso_invariant` proofs);
* `FirstOrder.initSeg` and `FirstOrder.ncard_initSeg`: the canonical encoding
  of `k ≤ n` as the initial segment of `Fin n`;
* `FirstOrder.ncard_compl_eq`: complement cardinality (`n - k`), as used by
  the Vertex Cover ↔ Independent Set reductions;
* `FirstOrder.ncard_tagged_eq_sum`: cardinality under tag-disjoint union —
  the tagged framework's *addition*;
* `FirstOrder.ncard_univ_pi`: cardinality of a product of marked sets over
...
representation of `FOReduction.Numbers.Binary` instead.
-/
