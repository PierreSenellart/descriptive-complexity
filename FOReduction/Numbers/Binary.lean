...
`FOReduction.Numbers.Unary` would change the complexity.

* `FirstOrder.posRank`: the rank of a position in the increasing enumeration
  (via `monoEquivOfFin`, so positions need not literally be `Fin m`);
* `FirstOrder.binValue b`: the number `∑ 2 ^ rank p` over set bits — the
  decoding function;
* `FirstOrder.binEncode k`: the canonical encoding, via `Nat.testBit`;
* round-trips `FirstOrder.binValue_binEncode` (for `k < 2 ^ #positions`) and
  `FirstOrder.testBit_binValue`;
* range bound `FirstOrder.binValue_lt_two_pow`;
* invariance under order-isomorphisms (`FirstOrder.binValue_orderIso`), which
  feeds `DecisionProblem.iso_invariant` proofs;
* the most-significant-differing-bit comparison
  `FirstOrder.binValue_lt_binValue_iff` — the Lean counterpart of the FO(≤)
