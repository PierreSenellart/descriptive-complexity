...
* A CNF formula is a `FirstOrder.Language.sat`-structure: elements are clauses
  ...
  clause `c`. `FirstOrder.SatSatisfiable` is the usual satisfiability.
* A (directed) graph is a `FirstOrder.Language.graph`-structure (already in
  Mathlib); `FirstOrder.ThreeColorableStructure` is properness of some
  3-coloring. On structures arising from Mathlib's `SimpleGraph` this agrees
  with `SimpleGraph.Colorable` (`FirstOrder.threeColorableStructure_iff_colorable`).
-/
...
def SAT : StructureProp Language.sat := fun A inst => @SatSatisfiable A inst
...
def ThreeColorableStructure : Prop :=
  ∃ c : V → Fin 3, ∀ x y : V, RelMap adj ![x, y] → c x ≠ c y

end Graph

/-- 3-colorability, as a problem on `Language.graph`-structures. -/
def ThreeCol : StructureProp Language.graph := fun V inst => @ThreeColorableStructure V inst

/-- On the first-order structure associated to a simple graph,
`ThreeColorableStructure` coincides with Mathlib's `SimpleGraph.Colorable 3`. -/
theorem threeColorableStructure_iff_colorable {V : Type} (G : SimpleGraph V) :
    @ThreeColorableStructure V G.structure ↔ G.Colorable 3 := by
  letI := G.structure
  constructor
  · rintro ⟨c, hc⟩
