...
(`FirstOrder.SigmaSODefinable.of_orderedReduction`,
`FirstOrder.PiSODefinable.of_orderedReduction`).

Pulling the defining sentence back through the interpretation
(`FOReduction.SecondOrderPull`) yields a sentence over the *ordered*
...
* the first block is extended with one binary relation variable, the order
  (`FirstOrder.SOBlock.withOrder`);
* the sentence is transported along the language morphism
  `FirstOrder.orderElimLHom` mapping the order symbol to the new variable;
* it is guarded by the first-order sentence `FirstOrder.linearGuard` stating
  that the variable is a linear order — as a conjunct if the block is
  existential (`Σₖ₊₁`), as a premise if it is universal (`Πₖ₊₁`).

Correctness of the guard uses `FirstOrder.linearOrderOfGuard` to promote a
