...
(`FirstOrder.sat_hard_of_sigmaSODefinable`, in
`FOReduction.Problems.Sat.Hardness`): the generic, machine-free reduction of
...
* `FirstOrder.Tseitin.NodeAt f m`: the type of subformula positions of `f`
  with context length `m` (the root is uniformly reachable via
  `FirstOrder.Tseitin.rootAt`);
* `FirstOrder.Tseitin.Gates`: a valuation of the position variables computes
  truth values correctly at every position ("all Tseitin gates hold");
* `FirstOrder.Tseitin.canonVal`, the canonical valuation by actual truth
  values, which satisfies all gates (`FirstOrder.Tseitin.gates_canonVal`),
  and the converse reading (`FirstOrder.Tseitin.gates_realize`): any
  valuation satisfying the gates assigns the root its actual truth value;
* `FirstOrder.Tseitin.IsClauseSem` / `FirstOrder.Tseitin.LitSem`: the clauses
...
* the main equivalence `FirstOrder.Tseitin.satCond_iff_gates`: a valuation
...
`FOReduction.Problems.Sat.TseitinFormulas`.
-/
...
(`FirstOrder.Tseitin.gates_realize`). -/
...
The clause-existence guards of `FirstOrder.Tseitin.IsClauseSem` are not
repeated here. -/
def LitSem (s : Bool) :
    ∀ {n : ℕ} (f : (L.sum B.lang).BoundedFormula Empty n), maxCtx f ≤ D →
