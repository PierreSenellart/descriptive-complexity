...
-/
import FOReduction.Problems.Sat.Tseitin
import FOReduction.Ordered

/-!
# The Tseitin encoding of a first-order kernel: defining formulas

First-order counterpart of `FOReduction.Problems.Sat.Tseitin`: formulas over
...
literals — `FirstOrder.Tseitin.isClauseF` mirroring
`FirstOrder.Tseitin.IsClauseSem` and `FirstOrder.Tseitin.litF` mirroring
`FirstOrder.Tseitin.LitSem`, with realization lemmas
(`FirstOrder.Tseitin.realize_isClauseF`, `FirstOrder.Tseitin.realize_litF`).
...
single place: the formula `FirstOrder.Tseitin.botF` expressing that a
...
`FirstOrder.Tseitin.IsClauseSem`: the clause tuple is read from the
...
`FirstOrder.Tseitin.LitSem`: the clause tuple is read from the coordinates
selected by `u`, the literal tuple from those selected by `x`. Root positions
of subformulas are recognized by `FirstOrder.Tseitin.isRootB`. -/
