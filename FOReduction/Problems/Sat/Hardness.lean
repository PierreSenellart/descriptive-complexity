...
import FOReduction.Problems.Sat.TseitinFormulas
...
first-order reduction to SAT (`FirstOrder.sat_hard_of_sigmaSODefinable`, an
axiom in earlier versions of this development). Since NP is *defined* as
`Σ₁`-definability (`FOReduction.Hierarchy`), together with the membership
half `FirstOrder.sat_sigmaSODefinable` this makes SAT NP-complete
(`FirstOrder.SAT_NP_complete`) — with the three PTIME axioms of
`FOReduction.Hierarchy` as the only remaining axiomatic basis.
...
(`FOReduction.Problems.Sat.Tseitin`):
...
* tuples of length `FirstOrder.tseitinDim B φ` are padded canonically with
...
The correctness proof (`FirstOrder.tseitin_satisfiable_iff`) composes the
characterization lemmas of the interpreted relations with the semantic
equivalence `FirstOrder.Tseitin.satCond_iff_gates` and the gate-correctness
lemmas `FirstOrder.Tseitin.gates_realize` / `FirstOrder.Tseitin.gates_canonVal`.
-/
...
(`FirstOrder.Tseitin.litF`), and the root variable, at fully padded tuples,
...
generic Tseitin reduction `FirstOrder.tseitinReduction`; an axiom in earlier
...
`FirstOrder.sat_sigmaSODefinable`; hardness is
`FirstOrder.sat_hard_of_sigmaSODefinable`, the generic Tseitin reduction. -/
