...
* `FirstOrder.Clique`: some clique is at least as large as the marked set;
* `FirstOrder.IndependentSet`: some independent set is at least as large as
  the marked set;
* `FirstOrder.VertexCover`: some vertex cover is at most as large as the
  marked set.
...
The three predicates are instances of generic properties `FirstOrder.CliqueOn`
...
reductions of `FOReduction.Problems.CliqueFamily.Reductions`.
-/
