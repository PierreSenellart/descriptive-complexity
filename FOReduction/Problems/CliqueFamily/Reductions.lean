...
* `FirstOrder.indSet_fo_reduction_clique` and
  `FirstOrder.clique_fo_reduction_indSet`: complementing the edges (off the
  diagonal) exchanges cliques and independent sets, keeping the marked set;
* `FirstOrder.vertexCover_fo_reduction_indSet` and
  `FirstOrder.indSet_fo_reduction_vertexCover`: complementing the *marked
...
* composites `FirstOrder.vertexCover_fo_reduction_clique` and
  `FirstOrder.clique_fo_reduction_vertexCover`.

The counting step of the vertex-cover reductions
(`FirstOrder.coverOn_iff_indepOn_not`) is the only place where finiteness is
