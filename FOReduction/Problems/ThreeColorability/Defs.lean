...
Mathlib); `FirstOrder.ThreeColorable` is properness of some 3-coloring, and
`FirstOrder.ThreeCol` the bundled decision problem. On structures arising
from Mathlib's `SimpleGraph` this agrees with `SimpleGraph.Colorable`
(`FirstOrder.threeColorable_iff_colorable`).
