...
reduction — `FirstOrder.sat_ordered_fo_reduction_threeSat : SAT ≤ᶠᵒ[≤]
ThreeSAT`. Every clause of the input CNF is split along the linear order of
its literal occurrences (`FOReduction.OccurrenceOrder`) into a chain of
clauses of width at most three, linked by fresh variables.

Concretely, the interpretation (`FirstOrder.SatToThreeSat.satToThreeSat`,
